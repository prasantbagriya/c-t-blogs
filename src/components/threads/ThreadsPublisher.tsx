...
export const ThreadsPublisher = ({ user, account }: { user: any; account: any }) => {
 ...
 className="p-2 rounded-none border bg-slate-50 dark:bg-white/5 border-slate-100 dark:border-white/5 text-slate-400 hover:text-black dark:hover:text-white "
 title="Upload Media (Images/Videos)"
 >
 {uploading ? <RefreshCw size={14} className="" /> : <ImageIcon size={14} />}
 </button>
 </div>
 </div>
 ...
 className="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-none p-4 text-sm focus:ring-1 focus:ring-blue-500 outline-none dark:text-white min-h-[140px] resize-none font-medium"
 />

 {mediaUrls.length > 0 && (
 <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
 {mediaUrls.map((url, idx) => (
 <div key={idx} className="relative aspect-square rounded-none overflow-hidden border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/5 group">
 ...
 className="absolute top-1 right-1 p-1 bg-black/50 text-white rounded-none opacity-0 group-hover:opacity-100 hover:bg-rose-500"
 >
 <X size={12} />
 </button>
 </div>
 ...
 {error && (
 <div className="mt-3 p-3 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-500/20 rounded-none flex items-center gap-3 text-rose-600 text-[10px] font-bold">
 <AlertCircle size={14} />
 {error}
 </div>
 )}
 ...
 {success && (
 <div className="mt-3 p-3 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-500/20 rounded-none flex items-center gap-3 text-emerald-600 text-[10px] font-bold uppercase tracking-widest">
 ...
 className="w-full mt-4 py-3.5 bg-black dark:bg-white dark:text-black text-white rounded-none font-black text-[10px] uppercase tracking-widest hover:opacity-90 disabled:opacity-40 flex items-center justify-center gap-2 "
 >
 {loading ? <RefreshCw size={14} className="" /> : <Send size={14} />}
 ...
 <div className="w-10 h-10 bg-slate-100 dark:bg-white/10 rounded-none flex items-center justify-center overflow-hidden">
 ...
 {mediaUrls.map((url, i) => (
 <div key={i} className="aspect-square rounded-none overflow-hidden border border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-black/20">
 {url.toLowerCase().match(/\.(mp4|mov|webm)$/) ? (
