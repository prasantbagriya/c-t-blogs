...
export const ThreadsCallback = ({ user, loading, onComplete }: { user: any, loading?: boolean, onComplete?: () => void }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#0a0a0f] p-6">
      <div className="max-w-md w-full bg-white dark:bg-[#16161d] rounded-[2.5rem] p-10 border border-slate-200 dark:border-white/5 shadow-2xl text-center">
        ...
        {status === 'loading' && (
          <div className="space-y-4">
            <RefreshCw className="animate-spin text-black mx-auto" size={32} />
        ...
        {status === 'error' && (
          <div className="space-y-4">
            ...
            {(errorMsg.includes('[1]') || errorMsg.includes('Failed')) && (
              <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/30 rounded-2xl text-[10px] text-amber-800 dark:text-amber-200 text-left space-y-2">
                ...
                <ul className="list-disc pl-4 space-y-1">
                  ...
                  <li>In Meta Dashboard, go to <strong>Threads</strong> {'>'} <strong>Settings</strong> and add: <br/><code className="bg-black/10 px-1 rounded">{window.location.origin}/threads-callback</code> to the <strong>Redirect URIs</strong>.</li>
            ...
            <button 
              ...
              className="mt-6 px-8 py-3 bg-slate-100 dark:bg-white/5 rounded-2xl text-xs font-black uppercase tracking-widest"
            >
