...
export const ThreadsComments = ({ user, account }: { user: any, account: any }) => {
 ...
 <div className={cn(
 "w-full sm:w-72 border-r border-slate-100 dark:border-white/10 flex flex-col bg-white dark:bg-[#111118] ",
 ...
 className={cn("p-1.5 rounded-none ", showAutoManager ? "bg-emerald-500 text-white" : "text-slate-400 hover:text-emerald-500")}
 >
 <Bot size={14} />
 </button>
 <button onClick={() => setShowGuard(!showGuard)} title="Keyword Guard" className={cn("p-1.5 rounded-none ", showGuard ? "bg-blue-500 text-white" : "text-slate-400 hover:text-blue-500")}><Shield size={14} /></button>
 <button onClick={fetchThreads} className="p-1.5 text-slate-400 hover:text-black dark:hover:text-white "><RefreshCw size={14} className={loadingThreads ? '' : ''} /></button>
 </div>
 </div>
 <div className="flex-1 overflow-y-auto no-scrollbar">
 {threads.map((thread) => (
 <div key={thread.id} onClick={() => { setSelectedThread(thread); setView('chat'); }} className={cn("p-3.5 cursor-pointer border-b border-slate-50 dark:border-white/10 flex items-center justify-between group", selectedThread?.id === thread.id ? "bg-slate-50 dark:bg-white/10 border-l-2 border-l-black dark:border-l-white" : "hover:bg-slate-100/50 dark:hover:bg-white/[0.08]")}>
 ...
 {(thread.auto_reply_enabled || thread.auto_dm_enabled) && <Zap size={10} className="text-emerald-500 " />}
 </div>
 </div>
 ...
 <div className={cn(
 "flex-1 flex flex-col bg-slate-50 dark:bg-[#0d0d12] relative overflow-hidden ",
 ...
 <button onClick={() => setShowGuard(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-none text-slate-400"><X size={20} /></button>
 </div>
 <div className="px-2 py-4 flex-1 overflow-y-auto no-scrollbar space-y-6">
 <div className="bg-blue-500/5 border border-blue-500/10 rounded-none p-4 flex items-start gap-3"><Zap className="text-blue-500 shrink-0" size={18} /><p className="text-[11px] text-blue-600 font-bold capitalize">Auto-delete spam comments instantly.</p></div>
 <div className="space-y-3">
 <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Banned Words</label>
 <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 rounded-none border border-slate-200 dark:border-white/5">
 <div className="flex items-center gap-3">
 <div className={`w-10 h-10 rounded-none flex items-center justify-center ${autoDelete ? 'bg-rose-500 text-white -rose-500/20' : 'bg-slate-200 dark:bg-white/10 text-slate-400'}`}>
 <ShieldAlert size={20} />
 </div>
 <div>
 ...
 className={`w-12 h-6 rounded-none p-1 ${autoDelete ? 'bg-rose-500' : 'bg-slate-300 dark:bg-white/10'}`}
 >
 <div className={`w-4 h-4 bg-white rounded-none ${autoDelete ? 'translate-x-6' : 'translate-x-0'}`} />
 </button>
 </div>

 <div className="flex gap-2 mb-6">
 <input value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddKeyword()} placeholder="Add word..." className="flex-1 h-12 bg-slate-50 dark:bg-[#1a1a25] border border-slate-100 dark:border-white/10 rounded-none px-4 text-sm outline-none text-slate-900 dark:text-white" />
 <button onClick={handleAddKeyword} className="w-12 h-12 flex items-center justify-center bg-black dark:bg-white text-white dark:text-black rounded-none"><Plus size={20} /></button>
 </div>
 </div>
 <div className="flex flex-wrap gap-2 pb-10">
 {keywords.map((word) => (
 <div key={word} className="flex items-center gap-2 bg-slate-100 dark:bg-white/10 px-4 py-2 rounded-none border border-slate-200 dark:border-white/10"><span className="text-xs font-black text-slate-700 dark:text-slate-200 capitalize">{word}</span><button onClick={() => handleRemoveKeyword(word)} className="text-slate-400"><X size={14} /></button></div>
 ))}
 </div>
 </div>
 </motion.div>
 ...
 <button onClick={() => setShowAutoManager(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-none text-slate-400"><X size={20} /></button>
 </div>
 ...
 <div className="p-4 bg-slate-50 dark:bg-[#1a1a25] border border-slate-100 dark:border-white/10 rounded-none space-y-3">
 <span className="text-[11px] font-black text-blue-500">Trigger keyword</span>
 <input value={triggerKeyword} onChange={(e) => setTriggerKeyword(e.target.value)} placeholder="e.g. Link (Empty for all)..." className="w-full h-12 bg-white dark:bg-[#252535] border border-slate-200 dark:border-white/10 rounded-none px-4 text-sm outline-none text-slate-900 dark:text-white" />
 </div>
 <div className="space-y-3">
 <div className="flex items-center justify-between"><span className="text-[11px] font-black text-slate-900 dark:text-white">Public reply</span><button onClick={() => setIsAutoReplyEnabled(!isAutoReplyEnabled)} className={cn("w-12 h-6 rounded-none relative ", isAutoReplyEnabled ? "bg-blue-500" : "bg-slate-200 dark:bg-white/10")}><div className={cn("absolute top-1 w-4 h-4 bg-white rounded-none ", isAutoReplyEnabled ? "left-7" : "left-1")} /></button></div>
 <textarea value={autoReplyText} onChange={(e) => setAutoReplyText(e.target.value)} placeholder="Public reply..." className="w-full h-24 bg-slate-50 dark:bg-[#1a1a25] border border-slate-100 dark:border-white/10 rounded-none p-4 text-sm outline-none text-slate-900 dark:text-white resize-none" />
 </div>
 <div className="space-y-3">
 <div className="flex items-center justify-between"><span className="text-[11px] font-black text-slate-900 dark:text-white">Private DM</span><button onClick={() => setIsAutoDmEnabled(!isAutoDmEnabled)} className={cn("w-12 h-6 rounded-none relative ", isAutoDmEnabled ? "bg-emerald-500" : "bg-slate-200 dark:bg-white/10")}><div className={cn("absolute top-1 w-4 h-4 bg-white rounded-none ", isAutoDmEnabled ? "left-7" : "left-1")} /></button></div>
 <textarea value={autoDmText} onChange={(e) => setAutoDmText(e.target.value)} placeholder="Private DM..." className="w-full h-24 bg-slate-50 dark:bg-[#1a1a25] border border-slate-100 dark:border-white/10 rounded-none p-4 text-sm outline-none text-slate-900 dark:text-white resize-none" />
 <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-none space-y-4">
 <div className="flex items-center justify-between"><span className="text-[10px] font-black text-emerald-600">Attached resource</span><button onClick={() => fileInputRef.current?.click()} className="h-10 px-4 bg-white dark:bg-[#111118] dark:text-white rounded-none border border-emerald-200 dark:border-white/10 text-[10px] font-black uppercase">{isUploading ? <Loader2 size={14} className="" /> : <Paperclip size={14} />}</button></div>
 <input value={dmAttachmentUrl} onChange={(e) => setDmAttachmentUrl(e.target.value)} placeholder="Paste link or upload..." className="w-full h-10 bg-white dark:bg-[#252535] border border-emerald-200 dark:border-white/10 rounded-none px-3 text-[11px] outline-none text-slate-900 dark:text-white" />
 </div>
 </div>
 
 <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-none space-y-3">
 ...
 className="w-full h-12 bg-white dark:bg-[#1a1a25] border border-blue-200 dark:border-white/10 rounded-none px-4 text-sm outline-none text-slate-900 dark:text-white appearance-none cursor-pointer"
 >
 ...
 <button onClick={handleSaveAutomation} className="w-full h-12 bg-black dark:bg-white text-white dark:text-black rounded-none font-black text-sm ">Save automation</button>
 </div>
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <div className="w-8 h-8 bg-black dark:bg-[#252535] rounded-none flex items-center justify-center text-white dark:text-slate-200 text-[10px] font-black border border-slate-200 dark:border-white/10 overflow-hidden ">
 ...
 <button onClick={() => setShowAutoManager(true)} className={cn("flex items-center gap-2 px-3 py-2 rounded-none text-[10px] font-black uppercase ", (selectedThread.auto_reply_enabled || selectedThread.auto_dm_enabled) ? "bg-emerald-500 text-white" : "bg-slate-100 dark:bg-white/10 text-slate-500")}><Bot size={14} /><span className="hidden xs:inline">Automate</span></button>
 <a href={selectedThread.permalink} target="_blank" className="p-2 text-slate-400 border border-slate-100 dark:border-white/10 rounded-none"><ExternalLink size={16} /></a>
 </div>
 </div>
 <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar bg-slate-50 dark:bg-[#0d0d12]">
 {loadingReplies ? (<div className="flex flex-col items-center justify-center py-20"><RefreshCw className=" text-slate-300" size={24} /></div>) : (
 <div className="space-y-3">
 <h4 className="text-[9px] font-black capitalize text-slate-400 tracking-[0.2em] mb-4 px-1">Replies ({replies.length})</h4>
 {replies.map((reply) => (
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={reply.id} className="bg-white dark:bg-[#16161d] p-4 rounded-none border border-slate-100 dark:border-white/10 ">
 <div className="flex items-start gap-3">
 <div className="w-8 h-8 rounded-none border border-slate-100 dark:border-white/10 flex items-center justify-center text-[9px] font-black text-slate-400 overflow-hidden shrink-0">
 ...
 <div className="flex gap-2"><input value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder={`Reply to @${selectedThread.username}...`} className="flex-1 h-12 bg-slate-50 dark:bg-[#252535] border border-slate-100 dark:border-white/10 rounded-none px-4 text-sm outline-none text-slate-900 dark:text-white" /><button onClick={() => handlePostReply(selectedThread.id)} disabled={!replyText.trim() || sendingReply} className="w-12 h-12 flex items-center justify-center bg-black dark:bg-white text-white dark:text-black rounded-none ">{sendingReply ? <RefreshCw size={20} className="" /> : <Send size={20} />}</button></div>
 </div>
 </>
 ) : (
