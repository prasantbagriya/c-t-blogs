...
export const ThreadsInbox = ({ user, account }: { user: any, account: any }) => {
 ...
 <div className="w-16 h-16 bg-amber-500/10 text-amber-500 rounded-none flex items-center justify-center">
 <AlertCircle size={32} />
 </div>
 ...
 className="px-6 py-3 bg-blue-600 text-white rounded-none font-black text-[10px] tracking-widest -blue-500/20"
 >
 Reconnect account
 </button>
 <button 
 onClick={() => (window as any).setActiveTab?.('overview')}
 className="px-6 py-3 bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-white rounded-none font-black text-[10px] tracking-widest"
 >
 Go to Dashboard
 </button>
 </div>
 ...
 <button onClick={fetchConversations} className="p-2 text-slate-400 hover:text-blue-500 ">
 ...
 className={`p-4 cursor-pointer border-b border-slate-100 dark:border-white/5 flex items-center gap-3 ${selectedConversation?.id === conv.id ? 'bg-white dark:bg-[#16161d] ' : 'hover:bg-white/50 dark:hover:bg-white/5'}`}
 >
 <div className="w-10 h-10 bg-gradient-to-tr from-slate-200 to-slate-300 dark:from-slate-800 dark:to-slate-700 rounded-none flex items-center justify-center text-slate-500 dark:text-slate-400">
 <User size={20} />
 </div>
 ...
 <div className="flex items-center justify-center h-full"><RefreshCw className=" text-slate-200" /></div>
 ) : (
 messages.map(msg => {
 const isMe = msg.from?.id === account.threadsId;
 return (
 <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
 <div className={`max-w-[80%] p-3 rounded-none text-xs font-medium ${isMe ? 'bg-blue-600 text-white rounded-none' : 'bg-slate-100 dark:bg-[#16161d] text-slate-800 dark:text-slate-200 rounded-none'}`}>
 {msg.text}
 ...
 className="flex-1 bg-slate-100 dark:bg-[#16161d] border-none rounded-none px-4 py-3 text-xs font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-blue-500 outline-none"
 />
 <button 
 onClick={handleSend}
 disabled={!replyText.trim()}
 className="p-3 bg-blue-600 text-white rounded-none hover:scale-105 disabled:opacity-50 disabled:scale-100"
 >
 <Send size={18} />
 </button>
 </div>
