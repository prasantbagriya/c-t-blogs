...
export const ThreadsManager = ({ 
 ...
 <div className="w-20 h-20 bg-black rounded-none flex items-center justify-center mb-8 ">
 ...
 className="px-8 py-4 bg-black dark:bg-white text-white dark:text-black rounded-none font-black text-xs uppercase tracking-[0.2em] hover:opacity-90 flex items-center gap-3 "
 >
 Connect Now
 ...
 <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-[#16161d] rounded-none border border-transparent dark:border-white/5 overflow-x-auto no-scrollbar max-w-[calc(100vw-80px)] sm:max-w-none">
 {accounts.map(acc => (
 <button
 key={acc.id}
 onClick={() => setSelectedAccount(acc)}
 className={cn(
 "px-3 py-1.5 rounded-none text-[10px] font-black uppercase tracking-tight whitespace-nowrap",
 selectedAccount?.id === acc.id 
 ? "bg-white dark:bg-[#13131a] text-slate-900 dark:text-white " 
 ...
 <button 
 onClick={handleConnectThreads}
 className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-white hover:bg-black dark:hover:bg-white dark:hover:text-black rounded-none flex-none"
 title="Add New Threads Account"
 >
 <Plus size={14} />
 </button>
 </div>
 <button onClick={handleSyncProfile} disabled={isSyncing} className="p-2.5 bg-slate-100 dark:bg-white/5 rounded-none text-slate-400 hover:text-black dark:hover:text-white ">
 <RefreshCw size={16} className={isSyncing ? '' : ''} />
 </button>
 </div>
 ...
 <div key={i} className="bg-white dark:bg-[#16161d] p-4 rounded-none border border-slate-100 dark:border-white/5 ">
 <div className={`w-7 h-7 ${stat.color} text-white rounded-none flex items-center justify-center mb-2 `}>
 {stat.icon}
 </div>
 ...
 <div onClick={() => setActiveSubTab('publisher')} className="group cursor-pointer bg-black dark:bg-white p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 ...
 <div onClick={() => setActiveSubTab('inbox')} className="group cursor-pointer bg-emerald-600 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 <MessageSquare className="text-white mb-3" size={20} />
 <h3 className="text-xs font-black text-white uppercase">DM Inbox</h3>
 </div>
 <div onClick={() => setActiveSubTab('comments')} className="group cursor-pointer bg-blue-600 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 <MessageCircle className="text-white mb-3" size={20} />
 <h3 className="text-xs font-black text-white uppercase">Comments</h3>
 </div>
 <div onClick={() => setActiveSubTab('analytics')} className="group cursor-pointer bg-purple-600 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 <BarChart3 className="text-white mb-3" size={20} />
 <h3 className="text-xs font-black text-white uppercase">Insights</h3>
 </div>
 <div onClick={() => setActiveSubTab('settings')} className="group cursor-pointer bg-indigo-600 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 <Sparkles className="text-white mb-3" size={20} />
 <h3 className="text-xs font-black text-white uppercase">Auto Reply</h3>
 </div>
 <div onClick={() => setActiveSubTab('flow')} className="group cursor-pointer bg-slate-800 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 <Zap className="text-white mb-3" size={20} />
 <h3 className="text-xs font-black text-white uppercase">Flows</h3>
 </div>
 <div onClick={() => setActiveSubTab('spam')} className="group cursor-pointer bg-rose-600 p-4 rounded-none hover:scale-[1.02] relative overflow-hidden">
 ...
 className="w-24 h-24 rounded-none object-cover border-4 border-slate-50 dark:border-white/5 "
 alt="Profile"
 />
 <div className="absolute -bottom-1 -right-1 w-7 h-7 bg-emerald-500 rounded-none border-4 border-white dark:border-[#16161d]" />
 </div>
 <div>
 ...
 className="w-full sm:w-auto px-6 py-3 bg-black dark:bg-white text-white dark:text-black rounded-none font-black text-[10px] uppercase tracking-widest hover:opacity-90 flex items-center justify-center gap-2 "
 >
 <Edit2 size={14} />
 Edit Profile
 </button>
 ...
 disabled={isSyncing}
 className="w-full sm:w-auto px-6 py-3 bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-white rounded-none font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-white/10 flex items-center justify-center gap-2 border border-slate-200 dark:border-white/10 disabled:opacity-50"
 >
 <RefreshCw size={14} className={isSyncing ? "" : ""} />
 {isSyncing ? 'Syncing...' : 'Sync'}
 </button>
 <button 
 onClick={() => handleDeleteAccount(selectedAccount.id)}
 className="w-full sm:w-auto xs:col-span-2 sm:col-span-1 px-6 py-3 bg-rose-500/10 text-rose-600 rounded-none font-black text-[10px] uppercase tracking-widest hover:bg-rose-500 hover:text-white flex items-center justify-center gap-2"
 >
 <Trash2 size={14} />
 Remove
 </button>
 ...
 <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white dark:bg-[#111118] w-full max-w-md rounded-none border border-slate-200 dark:border-white/10 overflow-hidden">
 ...
 <button onClick={() => setShowProfileEditor(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-none text-slate-400"><Trash2 size={20} /></button>
 </div>
 ...
 className="w-full h-14 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-none flex items-center justify-center gap-3 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 "
 >
 {isUploadingPic ? <Loader2 size={18} className="" /> : <UploadCloud size={18} />}
 ...
 <textarea value={editBio} onChange={(e) => setEditBio(e.target.value)} placeholder="Tell the world about yourself..." className="w-full h-32 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-none p-4 text-sm outline-none text-slate-900 dark:text-white focus:border-blue-500 resize-none" />
 </div>
 <button 
 onClick={handleUpdateProfile}
 disabled={isUpdatingProfile}
 className="w-full h-14 bg-black dark:bg-white text-white dark:text-black rounded-none font-black text-xs hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-3"
 >
 {isUpdatingProfile ? <RefreshCw className="" size={16} /> : <Sparkles size={16} />}
 Update identity
 </button>
 </div>
 </motion.div>
 ...
 <div className="bg-white dark:bg-[#16161d] rounded-none border border-slate-200 dark:border-white/5 p-6 ">
 ...
 className="flex items-center justify-between p-5 bg-slate-50 dark:bg-black/20 rounded-none border border-slate-100 dark:border-white/5 cursor-pointer hover:bg-slate-100 dark:hover:bg-white/5 "
 >
 <div className="flex items-center gap-4">
 <div className={cn("w-12 h-12 rounded-none flex items-center justify-center ", selectedAccount?.ai_enabled ? "bg-blue-500 text-white" : "bg-slate-200 dark:bg-white/5 text-slate-400")}>
 <Sparkles size={24} />
 </div>
 <div>
 ...
 <div className={cn("w-12 h-6 rounded-none relative ", selectedAccount?.ai_enabled ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-700")}>
 <div className={cn("absolute top-1 w-4 h-4 bg-white rounded-none ", selectedAccount?.ai_enabled ? "right-1" : "left-1")} />
 </div>
 </div>

 {/* Handoff Timer */}
 <div className="p-5 bg-slate-50 dark:bg-black/20 rounded-none border border-slate-100 dark:border-white/5">
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 bg-purple-500/10 text-purple-500 rounded-none flex items-center justify-center">
 <RefreshCw size={16} />
 </div>
 ...
 className={cn(
 "flex-1 py-2 rounded-none text-[9px] font-black uppercase tracking-widest border ",
 (selectedAccount?.handoff_delay || 30) === m ? "bg-black dark:bg-white text-white dark:text-black border-black dark:border-white " : "border-slate-200 dark:border-white/5 text-slate-400 hover:bg-white dark:hover:bg-white/5"
 )}>
 {m}m
 </button>
 ))}
 ...
 <span className="px-3 py-1 bg-blue-500/10 text-blue-500 rounded-none text-[8px] font-bold uppercase tracking-widest">Local Sync</span>
 </div>
 ...
 className="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-none p-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-1 focus:ring-blue-500 outline-none min-h-[100px] resize-none font-medium"
 ...
 className="w-full py-3 bg-black dark:bg-white text-white dark:text-black rounded-none text-[10px] font-black uppercase tracking-widest hover:opacity-90 "
 >
 Update Identity
 </button>
 </div>
 ...
 <div key={i} className="p-4 bg-slate-50/50 dark:bg-black/20 rounded-none border border-slate-100 dark:border-white/5 flex items-center gap-3">
 <div className={cn("w-8 h-8 rounded-none flex items-center justify-center bg-white dark:bg-[#16161d] border border-slate-100 dark:border-white/5", item.color)}>
 {item.icon}
 </div>
 <div>
 ...
 {/* API Notice */}
 <div className="bg-amber-500/5 border border-amber-500/20 rounded-none p-4 flex items-start gap-4">
