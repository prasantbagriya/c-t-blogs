    ...
    <div className="flex flex-col gap-6 px-0 max-w-6xl">
      ...
      <div className="flex flex-col gap-1">
        {onBack && (
          <button 
            onClick={onBack}
            className="flex items-center gap-1.5 text-xs font-bold text-blue-500 hover:text-blue-600 transition-colors mb-2"
          >
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4 sm:gap-2">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="px-4 py-2.5 bg-slate-100 dark:bg-[#1a1a24] text-slate-700 dark:text-slate-200 text-sm font-medium rounded flex items-center gap-2 border border-slate-200 dark:border-white/5 transition-all"
            >
              <LayoutGrid size={16} />
            ...
            <button 
              ...
              className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded flex items-center gap-2 transition-colors shadow-lg shadow-blue-500/20"
            >
              <Plus size={16} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
              ...
              <div className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-xl overflow-hidden shadow-sm">
                  {isDeptLoading ? (
                    <div className="p-10 text-center text-[10px] font-bold text-slate-400 uppercase animate-pulse">Syncing...</div>
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-white/5">
                        {departments.map(dept => (
                            <div key={dept.id} className="p-3 flex items-center justify-between group hover:bg-slate-50 dark:hover:bg-[#1a1a24] transition-all">
                                <div className="flex items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-blue-500" />
                                    <span className="text-[11px] font-bold text-slate-700 dark:text-slate-200">{dept.name}</span>
                                </div>
                                <button onClick={() => handleDeleteDept(dept.id)} className="p-1 text-slate-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all">
                                    <Trash2 size={12} />
                                </button>
                            </div>
                        ))}
                  ...
                  <button onClick={handleAddDepartment} className="w-full p-3 text-[10px] font-bold text-blue-500 uppercase tracking-widest border-t border-slate-100 dark:border-white/5 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all">
                    + Create New
                  </button>
              </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
              ...
              {isLoading ? (
                ...
                <div className="py-20 bg-white dark:bg-[#16161d] border border-dashed border-slate-200 dark:border-white/10 rounded-2xl text-center shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {subUsers.map(u => (
                    <div key={u.uid} className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 p-5 rounded-2xl hover:border-blue-500 transition-all group shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                           <div className="flex items-center gap-3">
                              <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-[#1f1f2a] flex items-center justify-center font-bold text-slate-700 dark:text-slate-200 text-lg leading-none border border-slate-200 dark:border-white/5">
                                 {u.displayName?.[0]}
                              </div>
                              <div className="overflow-hidden">
                                 <h4 className="font-bold text-slate-900 dark:text-white truncate text-sm leading-tight flex items-center gap-2">
                                    {u.displayName}
                                    {u.department && <span className="px-1.5 py-0.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[8px] rounded uppercase font-bold">{u.department}</span>}
                                 </h4>
                           ...
                           <div className="flex gap-1">
                              <button 
                                ...
                                className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10 rounded-lg transition-all"
                              >
                                 <Edit2 size={16} />
                              </button>
                              <button 
                                onClick={() => handleDeleteUser(u.uid)}
                                className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/10 rounded-lg transition-all"
                              >
                                 <Trash2 size={16} />
                              </button>
                           </div>
                        ...
                        <div className="flex flex-col gap-3 pt-4 border-t border-slate-50 dark:border-white/5">
                           <div className="flex flex-wrap gap-1.5">
                              {u.permissions.features.map(f => (
                                <span key={f} className="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-[#1f1f2a] dark:text-slate-200 px-2 py-0.5 rounded uppercase tracking-tighter">
                                   {f}
                                </span>
                              ))}
                           </div>
      ...
      <AnimatePresence>
        {isFormOpen && (
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'tween' }} className="fixed inset-y-0 right-0 w-full md:w-[480px] bg-white dark:bg-[#13131a] z-[310] flex flex-col border-l border-slate-200 dark:border-white/10 shadow-2xl">
                <div className="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-white dark:bg-[#13131a]">
                    ...
                    <button onClick={() => setIsFormOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-all">
                        <X size={20} />
                    </button>
                </div>
                   ...
                   <div className="flex flex-col gap-4">
                      <div className="flex flex-col gap-1.5">
                         ...
                         <input required type="text" value={formData.displayName} onChange={e => setFormData({...formData, displayName: e.target.value})} placeholder="Agent Name"
                           className="w-full px-4 py-2.5 bg-slate-50 dark:bg-[#0f0f13] border border-slate-200 dark:border-white/10 rounded-xl text-sm font-medium focus:border-blue-500 outline-none transition-all dark:text-white"
                         />
                      </div>
                      <div className="flex flex-col gap-1.5">
                            ...
                            {FEATURES.map(f => (
                               <button key={f.id} type="button" onClick={() => toggleFeature(f.id)}
                                 className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                                   formData.permissions.features.includes(f.id)
                                     ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-500/50 text-blue-600 dark:text-blue-400 shadow-sm'
                                     : 'bg-white dark:bg-[#16161d] border-slate-200 dark:border-white/5 text-slate-500 dark:text-slate-200 hover:border-blue-500/30'
                                 }`}
                               >
                                  {f.icon}
                                  <span>{f.label}</span>
                           ...
                           <div className="grid grid-cols-2 gap-2">
                              {WHATSAPP_SUBFEATURES.map(sf => (
                                 <button key={sf.id} type="button" onClick={() => toggleSubFeature(sf.id)}
                                   className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border text-[10px] font-bold transition-all ${
                                     formData.permissions.subFeatures.whatsapp?.includes(sf.id)
                                       ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-500/50 text-emerald-600 dark:text-emerald-400 shadow-sm'
                                       : 'bg-white dark:bg-[#16161d] border-slate-200 dark:border-white/5 text-slate-500 dark:text-slate-200 hover:border-emerald-500/30'
                                   }`}
                                 >
                                    {sf.icon}
                            ...
                            {whatsappAccounts.map(acc => (
                               <button key={acc.id} type="button" onClick={() => toggleAccount(acc.id)}
                                 className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all ${
                                   formData.permissions.accounts.includes(acc.id)
                                     ? 'bg-slate-900 dark:bg-blue-600 border-transparent text-white shadow-lg shadow-black/10'
                                     : 'bg-white dark:bg-[#16161d] border-slate-200 dark:border-white/5 text-slate-500 dark:text-slate-200 hover:border-blue-500/30'
                                 }`}
                               >
                ...
                <div className="p-6 border-t border-slate-200 dark:border-white/10 flex gap-3 bg-white dark:bg-[#13131a]">
                    <button type="button" onClick={() => setIsFormOpen(false)} className="flex-1 py-3 px-4 bg-slate-100 dark:bg-[#1a1a24] text-slate-600 dark:text-slate-200 rounded-xl font-bold text-xs uppercase tracking-widest transition-all border border-transparent dark:border-white/5">Cancel</button>
                    <button type="submit" onClick={handleSaveUser} className="flex-[2] py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20">
