...
const FlowBuilderContent = ({ user, platform: propPlatform = 'whatsapp', selectedAccount, onOpenWidgetSettings }: { user: any, platform?: 'whatsapp' | 'instagram' | 'widget' | 'all', selectedAccount?: any, onOpenWidgetSettings?: () => void }) => {
  ...
  if (view === 'selection') {
    return (
      <div className="py-2 sm:py-12 px-1 sm:px-2 w-full max-w-7xl mx-auto space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="text-center space-y-2 sm:space-y-4">
          <button
            ...
            className="flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors mx-auto text-[10px] sm:text-xs font-bold uppercase tracking-widest"
          >
            <ArrowLeft size={12} /> {selectionStep === 'type' ? 'Back' : 'Cancel'}
          </button>
          
          <div className="flex items-center justify-center gap-1.5 mb-1 sm:mb-2">
            <div className={cn("w-1.5 h-1.5 rounded-full transition-all duration-300", selectionStep === 'account' ? "bg-blue-600 w-4" : "bg-blue-200")} />
            <div className={cn("w-1.5 h-1.5 rounded-full transition-all duration-300", selectionStep === 'type' ? "bg-blue-600 w-4" : "bg-blue-200")} />
          </div>
        ...
        {selectionStep === 'account' ? (
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 max-w-5xl mx-auto">
                ...
                {allAccounts.whatsapp.map(acc => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "group relative flex items-center gap-3 sm:gap-4 p-3 sm:p-5 bg-white dark:bg-[#16161d] border rounded-xl sm:rounded-2xl transition-all text-left shadow-sm",
                        isSelected ? "border-blue-500 ring-2 ring-blue-500/20" : "border-slate-200 dark:border-white/5"
                      )}
                    >
                      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-500 shrink-0 group-hover:scale-110 transition-transform">
                      ...
                      <div className="min-w-0 pr-6">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="px-1.5 py-0.5 bg-emerald-500 text-white text-[7px] sm:text-[8px] font-black rounded uppercase">WA</span>
                          <span className="text-[9px] sm:text-[10px] text-slate-400 font-medium truncate">{acc.phoneNumber}</span>
                        </div>
                      </div>
                      {isSelected && (
                        <div className="absolute top-2 right-2 sm:top-4 sm:right-4 w-4 h-4 sm:w-5 sm:h-5 bg-blue-600 rounded-full flex items-center justify-center text-white shadow-lg animate-in zoom-in-50">
                ...
                {allAccounts.instagram.map(acc => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-pink-500/10 flex items-center justify-center text-pink-500 shrink-0 group-hover:scale-110 transition-transform">
                ...
                {allAccounts.threads.map(acc => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-black flex items-center justify-center text-white shrink-0 group-hover:scale-110 transition-transform">
                        <Threads size={20} className="sm:w-7 sm:h-7" />
                      </div>
                      <div className="min-w-0 pr-6">
                        <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">@{acc.username}</h3>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="px-1.5 py-0.5 bg-black text-white text-[7px] sm:text-[8px] font-black rounded uppercase">TH</span>
                          <span className="text-[9px] sm:text-[10px] text-slate-400 font-medium truncate">{acc.displayName}</span>
                        </div>
                      </div>
                      {isSelected && (
                ...
                {allAccounts.website.map(acc => {
                  ...
                  return (
                    ...
                    >
                      <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-lg sm:rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-500 shrink-0 group-hover:scale-110 transition-transform">
                        <Globe size={20} className="sm:w-7 sm:h-7" />
                      </div>
                      <div className="min-w-0 pr-6">
                        <h3 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">{acc.name}</h3>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="px-1.5 py-0.5 bg-indigo-500 text-white text-[7px] sm:text-[8px] font-black rounded uppercase">Web</span>
            ...
            {tempSelectedAccounts.length > 0 && (
              <div className="flex justify-center pt-4 sm:pt-8 sticky bottom-6 sm:bottom-10 z-10 animate-in slide-in-from-bottom-4 duration-300 px-4">
                <button
                  onClick={() => setSelectionStep('type')}
                  className="w-full sm:w-auto flex items-center justify-center gap-3 px-6 sm:px-10 py-3 sm:py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-none font-bold text-xs sm:text-sm uppercase tracking-widest transition-all"
                >
            ...
            {allAccounts.whatsapp.length === 0 && allAccounts.instagram.length === 0 && (
              <div className="col-span-full py-20 text-center space-y-4">
                ...
                <button 
                  ...
                  className="px-6 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest"
                >
            ...
            {FLOW_TYPES.map((type) => (
              <div className="relative group w-full" key={type.id}>
                <button
                  ...
                  className="w-full flex items-center justify-between p-3 sm:p-5 bg-slate-50 dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-lg hover:border-blue-500 transition-all"
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div
                      className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center text-white transition-transform group-hover:scale-110"
  ...
  if (view === 'list') {
    return (
      <div className="py-2 sm:py-4 lg:py-6 px-0 sm:px-1 w-full space-y-4 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 pt-0 px-2 sm:px-0">
          <div className="flex items-center bg-[#1a1a24] border border-white/5 w-full sm:w-auto overflow-hidden rounded-xl">
            <button
              onClick={() => setActivePlatform('whatsapp')}
              className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all font-display ${
                activePlatform === 'whatsapp' ? "bg-blue-600 text-white" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
              }`}
            >
              WA Flows
            </button>
            ...
            <button
              ...
              className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all font-display ${
                activePlatform === 'instagram' ? "bg-blue-600 text-white" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
              }`}
            >
              IG Flows
            </button>
            <button
              ...
              className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all font-display ${
                activePlatform === 'threads' ? "bg-blue-600 text-white" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
              }`}
            >
              Threads Flows
            </button>
            <button
              ...
              className={`flex-1 sm:flex-none px-4 sm:px-8 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider transition-all font-display ${
                activePlatform === 'website' ? "bg-blue-600 text-white" : "text-slate-400 hover:text-slate-200 hover:bg-white/5"
              }`}
            >
              Web Flows
            </button>
          ...
          <button
            ...
            className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 sm:px-10 py-3 bg-blue-600 text-white text-[10px] font-bold uppercase tracking-wider transition-all hover:bg-blue-700 active:scale-95 font-display shadow-lg shadow-blue-600/20 rounded-xl"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-6 px-2 sm:px-0">
          {globalStats.map((stat, i) => (
            <div key={i} className="p-3 sm:p-4 bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-xl hover:border-blue-500/30 transition-all group shadow-sm">
              <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                <div className={cn("w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110", stat.bg, stat.color)}>
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/5 px-2 py-2">
            {displayFlows.length === 0 ? (
              ...
              displayFlows.map((flow) => (
                <div key={flow.id} className="px-2 py-1.5 flex flex-col sm:flex-row sm:items-center justify-between gap-1 hover:bg-slate-50 dark:hover:bg-[#1a1a24] transition-colors group rounded-xl border border-transparent hover:border-slate-200 dark:hover:border-white/10">
                  <div className="flex items-center gap-1">
                    <div
                      className="w-9 h-9 rounded-lg flex items-center justify-center text-white transition-all group-hover:scale-110"
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[8px] font-medium uppercase tracking-widest ${flow.status === 'Active' ? 'bg-emerald-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                          {flow.status}
                        </span>
                        ...
                        {flow.platform === 'whatsapp' && flow.whatsappAccountId && (
                          <span className="px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-widest bg-green-500/10 text-green-500 border border-green-500/20">
                            WA
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 transition-all">
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={(e) => { e.preventDefault(); openAnalytics(flow); }}
                        className="p-3 bg-slate-50 dark:bg-white/5 text-slate-500 hover:text-blue-500 rounded-xl transition-all"
                      >
                        <BarChart3 size={18} />
                      </button>
                      <button
            ...
            <div className="bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 text-xs font-bold uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmAndDelete}
                  className="flex-1 px-4 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold uppercase tracking-widest transition-all active:scale-95"
                >
                  Delete
                </button>
              </div>
  ...
  if (view === 'analytics' && selectedFlow) {
    ...
    return (
      <div className="py-3 sm:py-4 lg:py-6 px-0 w-full space-y-8 animate-in fade-in duration-500">
        <div className="flex items-center gap-4">
          <button type="button" onClick={() => setView('list')} className="p-3 bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 text-slate-500 rounded-xl hover:bg-slate-50 dark:hover:bg-white/10 dark:hover:text-white transition-all">
            <ArrowLeft size={18} />
          </button>
          <div>
        ...
        {flowMessages.length === 0 ? (
          <div className="p-20 text-center bg-slate-50 dark:bg-[#1a1a24] rounded-3xl border border-dashed border-slate-200 dark:border-white/10 space-y-4">
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {stats.map((stat, i) => (
                <div key={i} className="p-6 bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-3xl transition-all group">
                  <div className="flex items-center gap-3 mb-4">
                    <div className={cn("w-10 h-10 rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-12", stat.bg, stat.color)}>
                      {stat.icon}
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-3xl p-8">
                ...
                <div className="h-48 flex items-end gap-3 px-2">
                  {hourlyActivity.map((count, i) => (
                    <div key={i} className="flex-1 bg-blue-500/10 rounded-t-xl group relative cursor-pointer hover:bg-blue-500/40 transition-all" style={{ height: `${(count / maxActivity) * 100}%` }}>
                      <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[9px] font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 shadow-xl">
                        {count} Events
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-3xl p-8 flex flex-col">
                <h3 className="text-[10px] font-medium text-slate-900 dark:text-white uppercase tracking-[0.2em] mb-8 flex items-center gap-3">
                ...
                <div className="flex-1 overflow-y-auto p-4 pb-32 space-y-6 custom-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
                  {flowMessages.slice(-8).reverse().map((msg, i) => (
                    <div key={i} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-[#1a1a24] border border-slate-100 dark:border-white/5 hover:border-blue-500/30 transition-all rounded-2xl">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-white dark:bg-[#16161d] flex items-center justify-center font-medium text-xs text-blue-500 border border-slate-100 dark:border-white/5">
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          <button
            type="button"
            onClick={() => setView('list')}
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-50 dark:bg-[#1a1a24] flex items-center justify-center text-slate-500 hover:bg-slate-100 dark:hover:bg-[#252533] transition-all border border-slate-100 dark:border-white/5 shrink-0"
          >
            <ArrowLeft size={16} />
          </button>
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className={`shrink-0 text-[8px] font-medium uppercase px-2 py-0.5 rounded-full border ${currentStatus === 'Active'
                  ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                  : 'bg-slate-100 dark:bg-[#1a1a24] text-slate-500 border-slate-200 dark:border-white/10'
                }`}>
                {currentStatus}
              </span>
            </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            ...
            className="lg:hidden w-9 h-9 flex items-center justify-center text-slate-500 bg-slate-50 dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 rounded-xl hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all"
            title="Node Panel"
          >
            <Settings size={16} />
          </button>
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 bg-white dark:bg-[#1a1a24] text-slate-600 dark:text-slate-200 rounded-xl font-medium text-xs hover:bg-slate-50 dark:hover:bg-[#252533] transition-all disabled:opacity-50 border border-slate-200 dark:border-white/10"
          >
            <Save size={14} />
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-2 sm:py-2.5 bg-blue-600 text-white rounded-xl font-medium text-xs hover:bg-blue-700 transition-all disabled:opacity-50"
          >
            <Play size={14} />
      ...
      <div className="flex-1 flex overflow-hidden" style={{ minHeight: 0 }}>
        ...
        <aside
          className={cn(
            "bg-white dark:bg-[#13131a] border-r border-slate-200 dark:border-white/5 flex flex-col shrink-0 transition-all duration-300 overflow-hidden",
            // Desktop styles
            "lg:relative lg:translate-x-0",
            isSidebarCollapsed ? "lg:w-20" : "lg:w-72",
            // Mobile styles — fixed drawer
            "fixed top-0 left-0 h-full z-50 w-[300px] lg:static",
            isToolbarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full lg:translate-x-0"
          )}
          style={{ height: '100%' }}
        >
          ...
          <button
            ...
            className="hidden lg:flex absolute -right-3 top-10 w-6 h-6 bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/10 rounded-full items-center justify-center text-slate-500 hover:text-blue-500 shadow-sm z-50 transition-all"
          >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/5 shrink-0 lg:hidden">
            ...
            <button
              type="button"
              onClick={() => setIsToolbarOpen(false)}
              className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-700 dark:hover:text-white bg-slate-100 dark:bg-white/5 rounded-lg transition-colors"
            >
              <X size={16} />
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      "group w-full flex items-center gap-3 p-3 bg-white dark:bg-[#1a1a24] border border-slate-100 dark:border-white/5 rounded-xl hover:border-blue-500/50 hover:bg-blue-50/30 dark:hover:bg-blue-500/5 transition-all text-left",
                      isSidebarCollapsed && "justify-center p-4"
                    )}
                  >
                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0", node.color)}>
            ...
            <div className={cn("space-y-3 w-full", isSidebarCollapsed && "flex flex-col items-center")}>
              ...
              <div className="grid grid-cols-1 gap-2 w-full">
                {[
                  ...
                  >
                    <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-transform group-hover:scale-110", node.color)}>
                      {React.cloneElement(node.icon as React.ReactElement, { size: 16 })}
                    </div>
                    {!isSidebarCollapsed && (
        ...
        <div className="flex-1 relative w-full h-full min-h-0" style={{
          ...
          >
            ...
            <Panel position="top-right" className="bg-white/70 dark:bg-[#13131a]/70 backdrop-blur-md px-2.5 py-1 rounded-full border border-slate-200/50 dark:border-white/5 flex items-center gap-3 mt-16 mr-2">
              ...
              <button
                ...
                className="ml-2 p-1.5 text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-all"
            ...
            <Panel position="bottom-center" className="sm:hidden flex items-center gap-2 bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/10 rounded-2xl px-4 py-2 shadow-xl mb-2">
              <button
                type="button"
                onClick={() => setIsToolbarOpen(true)}
                className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-medium"
              >
              ...
              <button
                ...
                onClick={(e) => { e.preventDefault(); handleStatusUpdate('Draft'); }}
                disabled={isSaving}
                className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-300"
              >
                <Save size={14} /> Draft
              </button>
              <button
                ...
                onClick={(e) => { e.preventDefault(); handleStatusUpdate('Active'); }}
                disabled={isSaving}
                className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold"
              >
                <Play size={14} /> Deploy
              </button>
            </Panel>
