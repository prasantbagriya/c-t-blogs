...
const PropFirm = () => {
 ...
 {/* Settings */}
 <div className="bg-[#0a0a0a] border border-[#222] p-6 rounded-none grid grid-cols-2 gap-6">
 <div>
 <label className="text-sm font-medium text-gray-400 mb-2 block">Profit Target ($)</label>
 <input
 type="number"
 className="w-full bg-[#111] border border-[#333] rounded-none p-3 text-white focus:outline-none focus:border-indigo-500 "
 ...
 <select
 className="w-full bg-[#111] border border-[#333] rounded-none p-3 text-white focus:outline-none focus:border-indigo-500 appearance-none"
 value={threshold}
 ...
 {/* Add Day */}
 <div className="bg-[#0a0a0a] border border-[#222] p-6 rounded-none">
 <label className="text-sm font-medium text-gray-400 mb-2 block">Add Daily Profit</label>
 <div className="flex gap-4">
 <input
 type="number"
 className="flex-1 bg-[#111] border border-[#333] rounded-none p-3 text-white focus:outline-none focus:border-indigo-500 "
 ...
 <button
 onClick={addDay}
 className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-none font-semibold flex items-center"
 >
 ...
 <div key={idx} className="bg-[#1a1a1a] border border-[#333] px-4 py-2 rounded-none flex items-center gap-3 fade-in zoom-in ">
 <span className="text-sm">Day {idx + 1}: <span className="font-mono font-bold text-green-400">₹{p.toLocaleString()}</span></span>
 <button onClick={() => removeDay(idx)} className="text-gray-500 hover:text-red-400 ">
 <X className="w-4 h-4" />
 </button>
 </div>
 ))}
 ...
 {/* Status Card */}
 <div className={`p-6 rounded-none border ${stats.status === 'Safe' ? 'bg-green-500/10 border-green-500/20' : stats.status === 'Warning' ? 'bg-amber-500/10 border-amber-500/20' : 'bg-[#0a0a0a] border-[#222]'}`}>
 ...
 <div className="h-2 bg-gray-800 rounded-none overflow-hidden mt-2">
 <div
 ...
 <div className="bg-amber-500/5 border border-amber-500/20 p-6 rounded-none">
