...
export const WhatsAppPreview = ({ 
  ...
  const formatContent = (text: string) => {
    ...
    let parts: (string | React.ReactNode)[] = text.split(/(\{\{\d+\}\})/g).map((part, i) => {
      if (typeof part === 'string' && part.match(/\{\{\d+\}\}/)) {
        return <span key={`v-${i}`} className="bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 px-1 rounded font-bold">{part}</span>;
      }
      return part;
    });
  ...
  return (
    <div className="w-full max-w-[300px] mx-auto bg-[#e5ddd5] dark:bg-[#0b141a] rounded-[2.5rem] border-[8px] border-slate-900 dark:border-[#202c33] aspect-[9/18.5] overflow-hidden relative shadow-2xl flex flex-col scale-[0.95] origin-top">
      {/* Phone Notch */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-28 h-6 bg-slate-900 dark:bg-[#202c33] rounded-b-2xl z-20 flex items-center justify-center">
      ...
      {/* WhatsApp Header */}
      <div className="bg-[#075e54] dark:bg-[#202c33] pt-0 pb-3 px-4 flex items-center gap-3 relative z-10 shadow-md">
        <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-bold uppercase text-slate-600 border border-white/10">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-white text-[11px] font-bold flex items-center gap-1 truncate">
            ...
            {isVerified && (
              <span className="w-3 h-3 bg-blue-500 rounded-full flex items-center justify-center shadow-sm shrink-0">
      ...
      <div className="flex-1 p-3 space-y-3 overflow-y-auto bg-[url('https://chatwizs.com/assets/wa-bg-light.png')] dark:bg-none bg-repeat">
        <div className="bg-white dark:bg-[#202c33] rounded-none border-none p-0 max-w-[90%] relative shadow-sm overflow-hidden animate-in fade-in slide-in-from-left-2 duration-300">
        ...
        {buttons && buttons.length > 0 && (
          <div className="space-y-1 px-1 pb-4">
            {buttons.map((btn, idx) => (
              <div 
                key={idx} 
                className="bg-white/90 dark:bg-[#202c33]/90 backdrop-blur-sm py-2.5 px-4 rounded-none border border-white/10 shadow-sm flex items-center justify-center gap-2 text-blue-500 dark:text-blue-400 text-[10px] font-bold cursor-default hover:bg-white transition-colors"
              >
