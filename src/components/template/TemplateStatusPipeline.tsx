  ...
  return (
    <div className="w-full py-3">
      <div className="flex items-center justify-between relative">
        ...
        <div
          className="absolute top-1/2 left-0 h-[2px] transition-all duration-1000 -translate-y-1/2 z-0"
        ...
        {stages.map((stage, idx) => {
          ...
          return (
            <div key={stage.id} className="relative z-10 flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-500 ${isStageRejected ? 'bg-rose-500 text-white ring-4 ring-rose-50 dark:ring-rose-900/40' :
                  (isCompleted ? (idx === 3 ? 'bg-emerald-500 text-white ring-4 ring-emerald-50 dark:ring-emerald-900/40' : 'bg-blue-600 text-white ring-4 ring-blue-50 dark:ring-blue-900/40') : 'bg-white dark:bg-[#16161d] border-2 border-slate-200 dark:border-white/5 text-slate-700 dark:text-slate-200')
                }`}>
                {isStageRejected ? <AlertCircle size={14} /> : (isCompleted && !isCurrent ? <CheckCircle2 size={14} /> : <div className="w-2 h-2 rounded-full bg-current" />)}
              </div>
