...
export function IntegrationsView({ user, showToast, onNavigate }: { user: any, showToast: (m: string, t: any) => void, onNavigate?: (tab: string) => void }) {
 ...
 <div className="flex bg-slate-50 dark:bg-[#202c33] p-1 rounded-none border border-slate-200 dark:border-white/5">
 {[
 ...
 className={`px-4 py-2 rounded-none text-[10px] font-bold uppercase tracking-widest ${activeTab === tab.id ? 'bg-white dark:bg-blue-600 text-blue-600 dark:text-white' : 'text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
 >
 {tab.label}
 </button>
 ))}
