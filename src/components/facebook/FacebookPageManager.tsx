      ...
      <header className="bg-white dark:bg-[#0f0f11] border-b border-slate-200/60 dark:border-white/5 px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div className="flex items-center gap-6">
            <button 
              onClick={onBack}
              className="w-12 h-12 flex items-center justify-center bg-slate-50 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 rounded-2xl text-slate-500 hover:text-blue-600 transition-all shadow-sm"
            >
              <ChevronLeft size={24} />
            </button>
            
            <div className="relative">
              <div className="w-20 h-20 rounded-[2.5rem] bg-gradient-to-tr from-blue-600 to-indigo-400 p-1 shadow-2xl">
                {page?.picture?.data?.url ? (
                  <img src={page.picture.data.url} className="w-full h-full rounded-[2.2rem] object-cover border-4 border-white dark:border-[#0f0f11]" />
                ) : (
                  <div className="w-full h-full rounded-[2.2rem] bg-blue-600 flex items-center justify-center border-4 border-white dark:border-[#0f0f11]">
                    <Facebook size={32} className="text-white" />
                  </div>
                )}
              </div>
              <div className="absolute -bottom-2 -right-2 w-8 h-8 bg-blue-600 rounded-xl flex items-center justify-center text-white border-2 border-white dark:border-[#0f0f11] shadow-lg">
                <Facebook size={14} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1.5">
                <span className="text-[10px] font-black text-blue-600 bg-blue-50 dark:bg-blue-500/10 px-2.5 py-1 rounded-lg uppercase tracking-widest border border-blue-500/10">
          ...
          <div className="flex items-center gap-3">
             <button 
              onClick={loadData}
              className="px-6 py-3 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm"
             >
               Refresh
             </button>
             <a 
              href={page?.link} 
              target="_blank" 
              className="px-6 py-3 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-500/20 transition-all flex items-center gap-2"
             >
        ...
        <div className="max-w-6xl mx-auto flex gap-10">
          {[
            ...
            <button
              ...
              className={`py-6 text-[10px] font-black uppercase tracking-[0.25em] flex items-center gap-2 border-b-2 transition-all relative ${
                activeTab === tab.id 
        ...
        <div className="max-w-6xl mx-auto">
          <AnimatePresence mode="wait">
            {activeTab === 'feed' && (
              ...
              >
                {feed.length === 0 ? (
                  <div className="col-span-full py-40 text-center bg-white dark:bg-white/2 rounded-[3rem] border border-slate-200/60 dark:border-white/5">
                  ...
                  feed.map((post) => (
                    <div 
                      key={post.id}
                      className="bg-white dark:bg-[#111114] border border-slate-200/60 dark:border-white/5 rounded-[2.5rem] overflow-hidden hover:shadow-2xl transition-all group flex flex-col"
                    >
                      {post.full_picture && (
                        <div className="aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-white/2">
                          <img src={post.full_picture} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
                        </div>
                      )}
                      <div className="p-8 flex-1 flex flex-col">
                        <div className="flex justify-between items-center mb-6">
                           ...
                           <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-white/5 text-slate-400 hover:text-blue-600 transition-colors">
                        ...
                        <div className="mt-auto pt-6 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
                          <div className="flex gap-4">
                            <div className="flex items-center gap-2 px-3 py-2 bg-rose-50 dark:bg-rose-500/5 rounded-xl text-rose-600">
                            ...
                            <button 
                              ...
                              className="flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-500/5 rounded-xl text-blue-600 hover:bg-blue-600 hover:text-white transition-all"
                            >
                          ...
                          <a 
                            ...
                            target="_blank" 
                            className="w-10 h-10 flex items-center justify-center bg-slate-50 dark:bg-white/5 rounded-xl text-slate-400 hover:text-blue-600 transition-all border border-slate-200/50 dark:border-white/5"
                          >
            ...
            {activeTab === 'posts' && (
              ...
              >
                <div className="bg-white dark:bg-[#111114] border border-slate-200/60 dark:border-white/5 rounded-[3rem] p-10 shadow-2xl">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-3">
                      ...
                      <textarea 
                        ...
                        className="w-full h-56 bg-slate-50 dark:bg-white/2 border border-slate-200 dark:border-white/10 rounded-3xl p-8 text-base font-medium focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all resize-none shadow-inner"
                      />
                    </div>
                    <div className="space-y-3">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 dark:bg-white/2 border border-slate-200 dark:border-white/10 rounded-2xl px-8 py-5 text-sm font-medium focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all shadow-inner"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-6 bg-blue-600 text-white rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] hover:bg-blue-700 shadow-2xl shadow-blue-500/30 transition-all disabled:opacity-50 flex items-center justify-center gap-4"
                    >
                      {publishing ? <RefreshCw size={20} className="animate-spin" /> : <><Send size={20} /> Post to Page</>}
                    </button>
                  </div>
                </div>
              </motion.div>
              ...
              >
                {analytics.length === 0 ? (
                  ...
                  analytics.map((stat: any) => (
                    <div key={stat.name} className="bg-white dark:bg-[#111114] border border-slate-200/60 dark:border-white/5 rounded-[2.5rem] p-8 hover:shadow-xl transition-all group">
                       <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all mb-6">
                         <Zap size={20} />
                       </div>
        ...
        {selectedPost && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-5xl bg-white dark:bg-[#0a0a0b] rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row h-[85vh] border border-white/10"
            >
              ...
              <div className="w-full md:w-[450px] flex flex-col bg-white dark:bg-[#111114]">
                <header className="p-8 border-b border-slate-100 dark:border-white/5 flex justify-between items-center">
                   ...
                   <button onClick={() => setSelectedPost(null)} className="w-10 h-10 flex items-center justify-center bg-slate-50 dark:bg-white/5 rounded-xl text-slate-400 hover:text-black dark:hover:text-white transition-all"><Plus size={24} className="rotate-45" /></button>
                </header>
                    ...
                    comments.map(comment => (
                      <div key={comment.id} className="group">
                        <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-slate-200 to-slate-100 dark:from-white/10 dark:to-white/5 flex items-center justify-center text-xs font-black text-slate-600 dark:text-white uppercase">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-5 mt-3 ml-1">
                               <button className="text-[9px] font-black text-slate-400 hover:text-blue-500 uppercase tracking-widest transition-colors flex items-center gap-1.5">
                                 <Heart size={10} /> Like
                               </button>
                               <button className="text-[9px] font-black text-blue-500 hover:underline uppercase tracking-widest transition-colors">Reply</button>
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <footer className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/2">
                   <div className="relative">
                      <input 
                        ...
                        className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl pl-6 pr-14 py-5 text-sm font-medium focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all shadow-sm"
                        onKeyPress={(e) => e.key === 'Enter' && handleReply(comments[0]?.id)}
                      />
                      <button 
                        onClick={() => handleReply(comments[0]?.id)}
                        className="absolute right-2.5 top-2.5 w-12 h-12 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/30 flex items-center justify-center"
                      >
                        <Send size={18} />
                      </button>
                   </div>
