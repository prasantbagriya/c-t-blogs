...
export const ChatList = ({
  ...
  return (
    <aside className={`bg-white dark:bg-[#0a0a0f] border-r border-slate-100 dark:border-white/5 flex flex-col z-20 transition-all duration-300 h-full lg:relative lg:w-[320px] ${selectedChatId ? "hidden lg:flex" : "flex w-full lg:w-[320px]"}`}>
      <div className="p-4 space-y-4 bg-slate-50/50 dark:bg-black/20">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button 
              ...
              className={`p-2 rounded-xl transition-all ${showStats ? 'bg-black dark:bg-white text-white dark:text-black' : 'hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400'}`}
            >
              <Activity size={14} />
            </button>
            <button 
              onClick={() => setIsNewChatMode(!isNewChatMode)}
              className="p-2 bg-black dark:bg-white text-white dark:text-black rounded-xl transition-all hover:scale-105 active:scale-95"
            >
              <Plus size={14} className={isNewChatMode ? "rotate-45 transition-transform" : "transition-transform"} />
            </button>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-9 pr-3 py-2.5 bg-slate-100 dark:bg-[#16161d] border-none rounded-xl text-[11px] font-medium outline-none focus:ring-1 focus:ring-slate-200 dark:focus:ring-white/10 transition-all text-slate-900 dark:text-white placeholder:text-slate-500"
          />
        </div>

        {!hideSourceFilters && (
          <div className="flex gap-1 p-1 bg-slate-100 dark:bg-[#16161d] rounded-xl">
            {[
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all ${sourceFilter === s.id ? "bg-white dark:bg-[#1f1f27] text-black dark:text-white shadow-sm" : "text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"}`}
              >
                {s.icon}
                {s.label}
              </button>
      ...
      <AnimatePresence>
        {showStats && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 overflow-hidden mb-2">
            <div className="p-3 bg-white dark:bg-[#16161d] border border-slate-100 dark:border-white/5 rounded-2xl grid grid-cols-2 gap-2 text-center shadow-sm">
              <div>
        ...
        {chats.length === 0 ? (
          ...
          return (
            <button
              ...
              className={cn(
                "w-full p-3 rounded-xl flex items-center gap-3 transition-all duration-200 border-b border-transparent",
                isSelected 
                  ? "bg-white dark:bg-[#16161d] shadow-sm border-slate-100 dark:border-white/5" 
                  : "bg-transparent hover:bg-slate-50 dark:hover:bg-white/2"
              )}
            >
              <div className="relative flex-shrink-0">
                <div className={cn(
                    "w-11 h-11 rounded-full flex items-center justify-center font-black text-sm transition-colors overflow-hidden border-2 border-white dark:border-[#0a0a0f] shadow-md",
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <div className="flex justify-between items-center mt-0.5 gap-2">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {chat.unreadCount > 0 && (
                      <span className="flex h-4 min-w-[16px] px-1 items-center justify-center rounded-full bg-black dark:bg-white text-white dark:text-black text-[9px] font-black shadow-lg">
                        {chat.unreadCount}
                      </span>
                    )}
                    <div className={cn(
