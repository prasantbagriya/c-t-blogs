...
export const CampaignCard = ({ c, onClick }: { c: any; onClick: () => void; key?: any }) => {
  return (
    <button
      onClick={onClick}
      className="w-full text-left p-5 bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 rounded-xl hover:border-blue-500 transition-all group flex flex-col sm:flex-row sm:items-center justify-between gap-4"
    >
      <div className="flex items-center gap-4">
        <div className="w-10 h-10 rounded-lg bg-slate-50 dark:bg-[#16161d] flex items-center justify-center text-slate-700 dark:text-slate-200 group-hover:text-blue-500 transition-colors">
          <BarChart3 size={20} />
        </div>
        <div>
          <div className="flex wrap items-center gap-2">
            <h4 className="font-medium text-sm text-slate-900 dark:text-white uppercase tracking-tight">{c.name}</h4>
            <span className="px-2 py-0.5 bg-slate-100 dark:bg-[#16161d] text-slate-900 text-[10px] rounded-full border border-slate-200 dark:border-white/5 font-medium">
      ...
      <div className="flex items-center justify-between sm:justify-end gap-6 border-t sm:border-none border-slate-50 dark:border-white/5 pt-3 sm:pt-0">
          ...
          <div className="flex items-center gap-3">
              <span className={`px-2.5 py-1 ${c.status === 'Sent' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'} dark:bg-[#16161d] text-[8px] font-medium uppercase rounded-lg border border-current opacity-80 tracking-widest`}>
                  {c.status}
              </span>
...
export const CampaignInsightsPanel = ({ campaign, onClose }: { campaign: any, onClose: () => void }) => {
    ...
    return (
        <motion.div 
            ...
            className="fixed inset-y-0 right-0 w-full sm:w-[450px] bg-white dark:bg-[#16161d] shadow-none border-l border-slate-200 dark:border-white/10 z-[100] flex flex-col"
        >
            <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                <div className="flex-1 min-w-0 pr-4">
                    <div className="flex items-center gap-2">
                        <BarChart3 size={14} className="text-blue-600 flex-shrink-0" />
                ...
                <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-[#1a1a24] rounded-full transition-colors flex-shrink-0">
                    <X size={20} className="text-slate-700 dark:text-slate-200" />
                </button>
            </div>

            <div className="flex-1 overflow-y-auto px-3 py-4 sm:p-6 space-y-8">
                <div className="p-5 bg-gradient-to-r from-slate-900 to-blue-950 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-white/5">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/10 rounded-xl flex items-center justify-center border border-white/10 backdrop-blur-md">
                    ...
                    <button 
                        ...
                        disabled={loading}
                        className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 text-white rounded-lg border border-white/10 transition-all group disabled:opacity-50"
                    >
                ...
                <div className="grid grid-cols-2 gap-4">
                    {stats.map(s => (
                        <div key={s.label} className="p-5 border border-slate-100 dark:border-white/5 rounded-xl bg-white dark:bg-[#1a1a24] shadow-none hover:shadow-none transition-shadow">
                            ...
                            <div className="mt-4 h-1.5 w-full bg-slate-100 dark:bg-[#13131a] rounded-full overflow-hidden">
                                 <motion.div 
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-6 bg-slate-900 dark:bg-[#0a0a0e] rounded-2xl text-white border border-slate-800 dark:border-white/5 relative overflow-hidden group">
            ...
            <div className="p-6 bg-slate-50 dark:bg-[#13131a] border-t border-slate-200 dark:border-white/10 flex gap-3">
                <button 
                  ...
                  className="flex-1 py-2.5 bg-blue-600 text-white rounded font-medium text-xs hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
                >
                    ...
                    >
                        <header className="h-16 lg:h-20 border-b border-slate-100 dark:border-white/10 flex items-center justify-between px-3 lg:px-8 bg-white dark:bg-[#13131a] sticky top-0 z-10">
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                                <button onClick={() => setShowFullReport(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all text-slate-900 dark:text-slate-600">
                                    <ArrowLeft size={18} />
                                </button>
                                <div>
                            ...
                            <div className="flex items-center gap-4">
                                <button 
                                    ...
                                    className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg flex items-center gap-2 transition-all disabled:opacity-50"
                                >
                                    {isExporting ? <RefreshCw size={12} className="animate-spin" /> : <Plus size={12} />}
                                    <span>{isExporting ? 'Generating...' : 'Download PDF'}</span>
                                </button>
                                <button onClick={() => setShowFullReport(false)} className="p-2 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg">
                                    <X size={18} />
                                </button>
                            </div>
                        </header>
