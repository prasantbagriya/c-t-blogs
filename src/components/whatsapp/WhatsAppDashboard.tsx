...
const FeatureNode = ({ node, isActive, onClick }: any) => {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center justify-between p-1.5 bg-slate-50 dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-xl hover:border-blue-500 transition-all group ${isActive ? "border-blue-500 bg-white dark:bg-[#1a1a24] ring-2 ring-blue-500/10" : ""}`}
    >
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="px-3 pt-3 flex items-center justify-between">
          <div className="flex items-center gap-1 p-1 bg-white dark:bg-[#16161d] rounded-xl border border-slate-200 dark:border-white/5 overflow-x-auto no-scrollbar max-w-[calc(100vw-40px)] sm:max-w-none">
            {allAccounts?.map((acc: any) => (
              <button
                key={acc.id}
                onClick={() => setSelectedAccount(acc)}
                className={`px-3 py-1.5 rounded-lg transition-all text-[10px] font-black uppercase tracking-tight whitespace-nowrap ${
            ...
            <button 
              ...
              className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-emerald-500 hover:bg-emerald-500/10 rounded-lg transition-all flex-none border border-dashed border-slate-200 dark:border-white/10"
      ...
      <div className="grid grid-cols-3 gap-1.5 sm:gap-3 pt-0 px-3">
          <div className="flex items-center gap-1.5 sm:gap-3 px-2 sm:px-4 py-2 sm:py-2.5 bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 rounded-lg sm:rounded-xl">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 px-2 sm:px-4 py-2 sm:py-2.5 bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 rounded-lg sm:rounded-xl">
              <LayoutGrid size={14} className="text-emerald-500 flex-shrink-0" />
          ...
          <button 
            ...
            className="flex items-center justify-center gap-1.5 px-2 py-2 sm:py-2.5 bg-slate-900 text-white rounded-lg sm:rounded-xl text-[8px] sm:text-xs font-bold uppercase tracking-widest hover:bg-black transition-all"
          >
            <Plus size={14} className="flex-shrink-0" /> <span className="truncate">Add Dept</span>
          </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         <div className="lg:col-span-2 bg-white dark:bg-[#16161d] p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/5 flex flex-col sm:flex-row items-center gap-8 relative overflow-hidden group">
            <div className="absolute -right-10 -bottom-10 opacity-[0.03] group-hover:scale-110 transition-transform duration-700"><TrendingUp size={240} /></div>
            <div className="relative">
              <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full border-4 border-blue-500/10 flex items-center justify-center">
              ...
              <div className="absolute -top-1 -right-1 w-7 h-7 bg-blue-600 rounded-full flex items-center justify-center text-white shadow-lg border-2 border-white dark:border-[#16161d]"><Zap size={12} className="fill-current" /></div>
            </div>
            <div className="flex-1 space-y-5 relative z-10 w-full text-center sm:text-left">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button onClick={() => setActiveSubTab('leads')} className="w-full sm:w-auto px-6 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-bold hover:bg-blue-700 transition-all tracking-widest">CRM PIPELINE</button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[ 
                  ...
                  <div key={i} className="p-3 bg-slate-50 dark:bg-[#1a1a24] rounded-xl border border-slate-100 dark:border-white/5">
         ...
         <div className="bg-white dark:bg-[#16161d] p-6 rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col h-full min-h-[300px]">
            <div className="flex items-center justify-between mb-6">
              <p className="text-[10px] font-bold text-slate-600 tracking-wider uppercase">Live Activity Feed</p>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /><span className="text-[9px] font-bold text-emerald-500 uppercase">Live</span></div>
            </div>
            <div className="flex-1 space-y-5">
              {recentInbound.length === 0 ? (
                ...
                <div key={i} className="flex items-center gap-4 group cursor-pointer hover:translate-x-1 transition-transform" onClick={() => setActiveSubTab('inbox')}>
                  <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-[#1a1a24] border border-slate-100 dark:border-white/5 flex items-center justify-center text-xs font-bold text-slate-600 group-hover:text-blue-600 group-hover:bg-blue-50 transition-all">{msg.senderName?.[0] || 'U'}</div>
