...
export const MessageNode = ({ data, isConnectable }: any) => {
 ...
 {buttons.length > 0 && (
 <div className="flex items-center gap-1.5 py-1 px-2 bg-blue-50/30 dark:bg-blue-500/5 rounded-none border border-blue-100/50 dark:border-blue-500/10">
 <input
 type="checkbox"
 checked={data.smartBranching !== false}
 onChange={(e) => data.onChange('smartBranching', e.target.checked)}
 className="w-2.5 h-2.5 rounded-none border-slate-300 text-blue-500"
 />
 ...
 <div key={btn.id} className="relative group/btn space-y-1 p-2 bg-slate-50 dark:bg-[#252533] rounded-none border border-slate-100 dark:border-white/10 hover:border-blue-500/30">
 ...
 <div className="flex items-center gap-2">
 <select
 className="text-[7px] bg-slate-100 dark:bg-white/10 rounded-none px-1 outline-none font-bold text-slate-500"
 ...
 {btn.type === 'payment' && <span className="text-[7px] text-emerald-500 font-black uppercase">Auto-Link</span>}
 </div>
 </div>
 <button
 ...
 className="flex-1 text-[8px] bg-slate-100 dark:bg-white/5 rounded-none px-1 py-1 outline-none font-bold text-orange-600 cursor-pointer"
 >
 ...
 className="w-full text-[8px] p-1.5 bg-blue-50/50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/10 rounded-none outline-none text-blue-600 dark:text-blue-400 font-medium"
 placeholder="https://..."
 />
 </div>
 )}
