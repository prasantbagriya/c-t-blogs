...
export const GoogleSheetNode = ({ data, isConnectable }: any) => {
  return (
    <div className="px-4 py-3 bg-white dark:bg-[#1a1a24] rounded-lg border border-emerald-500/30 dark:border-emerald-500/50 min-w-[240px] shadow-lg shadow-emerald-500/5">
      ...
      <div className="space-y-3">
        <div className="space-y-1">
          ...
          <select
            ...
            className="w-full text-[10px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded outline-none font-bold"
          >
        ...
        <div className="space-y-1">
          ...
          <textarea
            ...
            rows={4}
            className="w-full text-[9px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded outline-none font-mono resize-none"
          />
          <div className="flex flex-wrap gap-1 mt-1">
            <span className="text-[7px] bg-blue-500/10 text-blue-500 px-1 rounded cursor-pointer" onClick={() => data.onChange('mapping', (data.mapping || '') + '{{name}}')}>{"{{name}}"}</span>
            <span className="text-[7px] bg-blue-500/10 text-blue-500 px-1 rounded cursor-pointer" onClick={() => data.onChange('mapping', (data.mapping || '') + '{{phone}}')}>{"{{phone}}"}</span>
            <span className="text-[7px] bg-blue-500/10 text-blue-500 px-1 rounded cursor-pointer" onClick={() => data.onChange('mapping', (data.mapping || '') + '{{amount}}')}>{"{{amount}}"}</span>
          </div>
        </div>
      </div>
