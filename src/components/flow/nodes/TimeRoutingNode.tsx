...
export const TimeRoutingNode = ({ data, isConnectable }: any) => (
  <div className="px-4 py-3 bg-white dark:bg-[#1a1a24] rounded-lg border border-amber-500/30 dark:border-amber-500/50 min-w-[240px] shadow-lg shadow-amber-500/5">
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          ...
          <input type="time" value={data.startTime || '09:00'} onChange={(e) => data.onChange('startTime', e.target.value)} className="w-full text-[10px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded outline-none font-bold text-slate-700 dark:text-slate-200" />
        </div>
        <div className="space-y-1">
          <p className="text-[8px] text-slate-500 uppercase font-bold px-1">End Time</p>
          <input type="time" value={data.endTime || '18:00'} onChange={(e) => data.onChange('endTime', e.target.value)} className="w-full text-[10px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded outline-none font-bold text-slate-700 dark:text-slate-200" />
        </div>
      </div>
      <div className="space-y-1">
        ...
        <div className="flex gap-1 justify-between px-1">
          {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((day, i) => {
            ...
            return (
              ...
              }} className={`w-6 h-6 rounded-md text-[8px] font-bold flex items-center justify-center transition-colors ${isActive ? 'bg-amber-500 text-white' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>{day[0]}</button>
            )
          })}
        </div>
      </div>
