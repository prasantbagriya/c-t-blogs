...
export const CRMUpdateNode = ({ data, isConnectable }: any) => (
 <div className="px-4 py-3 bg-white dark:bg-[#1a1a24] rounded-none border border-purple-500/30 dark:border-purple-500/50 min-w-[240px] -purple-500/5">
 ...
 <select value={data.provider || 'hubspot'} onChange={(e) => data.onChange('provider', e.target.value)} className="w-full text-[10px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded-none outline-none font-bold text-slate-700 dark:text-slate-200">
 ...
 <input type="password" value={data.apiKey || ''} onChange={(e) => data.onChange('apiKey', e.target.value)} placeholder="Enter API Key" className="w-full text-[10px] p-2 bg-slate-50 dark:bg-[#0f0f13] border border-slate-100 dark:border-white/5 rounded-none outline-none text-slate-700 dark:text-slate-200 font-mono" />
 </div>
 </div>
