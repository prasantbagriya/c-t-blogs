...
export const InstagramOptimizer = ({ user, account }: { user: any; account: any }) => {
 ...
 {/* Header Section */}
 <div className="bg-white dark:bg-[#16161d] p-6 rounded-none border border-slate-200 dark:border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
 <div className="flex items-center gap-4">
 <div className="w-16 h-16 rounded-none bg-blue-600/10 flex items-center justify-center text-blue-600">
 <Target size={32} />
 </div>
 <div>
 ...
 disabled={optimizing}
 className="w-full md:w-auto px-8 py-3 bg-blue-600 text-white rounded-none text-[11px] font-black uppercase tracking-[0.2em] hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-3 -blue-600/20"
 >
 {optimizing ? <RefreshCw size={16} className="" /> : <Sparkles size={16} />}
 {optimizing ? 'Analyzing Profile...' : 'Run SEO Audit'}
 </button>
 </div>

 {!suggestions ? (
 <div className="bg-white dark:bg-[#16161d] p-12 rounded-none border border-slate-200 dark:border-white/5 text-center space-y-4">
 <div className="w-20 h-20 bg-slate-50 dark:bg-white/5 rounded-none flex items-center justify-center mx-auto">
 ...
 className={`w-full p-4 rounded-none border flex items-center gap-3 ${ activeTab === tab.id ? 'bg-white dark:bg-[#1a1a24] border-blue-600 ' : 'bg-transparent border-transparent text-slate-400 hover:bg-white/5' }`}
 >
 <div className={`p-2 rounded-none ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-white/5'}`}>
 {tab.icon}
 </div>
 ...
 exit={{ opacity: 0, y: -10 }}
 className="bg-white dark:bg-[#16161d] p-8 rounded-none border border-slate-200 dark:border-white/5"
 >
 {activeTab === 'bio' && (
 <div className="space-y-6">
 <div className="space-y-2">
 <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Recommended Bio</h4>
 <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-none border border-slate-100 dark:border-white/5 relative group">
 <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-line">
 {suggestions.bio}
 </p>
 <button className="absolute top-4 right-4 p-2 opacity-0 group-hover:opacity-100 bg-white dark:bg-[#1a1a24] rounded-none text-blue-600 hover:scale-110">
 <Save size={14} />
 </button>
 </div>
 </div>
 <div className="grid grid-cols-2 gap-4">
 <div className="p-4 bg-emerald-500/5 rounded-none border border-emerald-500/20">
 ...
 <span key={kw} className="px-2 py-0.5 bg-white dark:bg-white/5 text-emerald-600 text-[8px] font-black rounded-none uppercase tracking-tighter border border-emerald-500/10">
 {kw}
 </span>
 ))}
 </div>
 </div>
 <div className="p-4 bg-blue-500/5 rounded-none border border-blue-500/20">
 ...
 <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-none border border-slate-100 dark:border-white/5 flex flex-wrap gap-2">
 ...
 <div key={idx} className="p-4 bg-white dark:bg-[#1a1a24] rounded-none border border-slate-100 dark:border-white/5 ">
 ...
 <div className="p-6 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-none text-white">
