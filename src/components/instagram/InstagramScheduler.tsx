...
export const InstagramScheduler = ({ user, account }: { user: any; account: any }) => {
 ...
 {/* Header */}
 <div className="flex items-center justify-between bg-white dark:bg-[#16161d] rounded-none border border-slate-200 dark:border-white/5 p-4 ">
 <div>
 ...
 <button onClick={fetchScheduled} disabled={loading} className="p-2 border border-slate-200 dark:border-white/5 rounded-none text-slate-400 hover:text-amber-500 ">
 <RefreshCw size={14} className={loading ? '' : ''} />
 </button>
 <button onClick={() => setShowForm(true)} className="flex items-center gap-2 px-6 py-3 bg-amber-500 text-white rounded-none text-[10px] font-black uppercase tracking-widest hover:opacity-90 -amber-500/20">
 ...
 className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-none p-6 space-y-5 ">
 ...
 <button onClick={() => setShowForm(false)} className="text-slate-400 hover:text-red-500 "><X size={16} /></button>
 </div>
 ...
 className={`flex-1 py-2 rounded-none text-[10px] font-bold uppercase tracking-widest border ${form.type === t ? 'bg-amber-500 text-white border-amber-500' : 'border-slate-100 dark:border-white/5 text-slate-400'}`}>
 {t}
 </button>
 ))}
 </div>
 ...
 className="border-2 border-dashed border-slate-200 dark:border-white/10 rounded-none p-6 text-center cursor-pointer hover:border-amber-400 group">
 {form.mediaPreview ? (
 form.type === 'reel'
 ? <video src={form.mediaPreview} className="h-32 mx-auto rounded-none object-cover" />
 : <img src={form.mediaPreview} className="h-32 mx-auto rounded-none object-cover" alt="" />
 ) : (
 ...
 className="w-full p-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-none text-xs outline-none focus:ring-2 focus:ring-amber-500 dark:text-white resize-none" />
 ...
 className="w-full p-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-none text-xs outline-none focus:ring-2 focus:ring-amber-500 dark:text-white" />
 </div>

 <div className="flex gap-2">
 <button onClick={() => setShowForm(false)} className="flex-1 py-2.5 border border-slate-200 dark:border-white/10 rounded-none text-xs font-bold text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5 ">Cancel</button>
 <button onClick={handleSchedule} disabled={submitting}
 className="flex-1 py-2.5 bg-amber-500 text-white rounded-none text-xs font-bold uppercase hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2">
 {submitting ? <RefreshCw size={14} className="" /> : <CalendarClock size={14} />}
 ...
 {/* Scheduled Posts */}
 {loading ? (
 <div className="flex justify-center py-12"><RefreshCw className="w-8 h-8 text-amber-500 " /></div>
 ) : scheduled.length === 0 ? (
 <div className="text-center py-16 bg-white dark:bg-[#16161d] rounded-none border border-slate-200 dark:border-white/5">
 ...
 className="bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 rounded-none p-4 flex items-center gap-4">
 {/* Media thumb */}
 <div className="w-14 h-14 rounded-none overflow-hidden bg-slate-100 dark:bg-white/10 shrink-0">
 {item.mediaUrl ? (
 item.type === 'reel'
 ...
 <span className={`px-2 py-0.5 rounded-none text-[8px] font-bold uppercase ${ item.type === 'reel' ? 'bg-pink-100 text-pink-600 dark:bg-pink-900/20 dark:text-pink-400' : item.type === 'story' ? 'bg-purple-100 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400' : 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400' }`}>{item.type}</span>
 ...
 <span className={`flex items-center gap-1 text-[9px] font-bold uppercase tracking-wide ${ item.status === 'published' ? 'text-emerald-500' : item.status === 'failed' ? 'text-red-500' : 'text-amber-500' }`}>
 ...
 <button onClick={() => handleCancel(item.id)} className="p-2 text-slate-400 hover:text-red-500 shrink-0">
 <Trash2 size={15} />
 </button>
 )}
 </motion.div>
