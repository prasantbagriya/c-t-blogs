...
export const InstagramComments = ({ user, account }: { user: any, account: any }) => {
 ...
 <div className={cn(
 "w-full sm:w-80 border-r border-slate-100 dark:border-white/5 flex flex-col bg-slate-50/50 dark:bg-black/20 ",
 ...
 <button onClick={() => setShowGuard(!showGuard)} className={cn("p-2 rounded-none ", showGuard ? "bg-pink-500 text-white -pink-500/20" : "text-slate-400 hover:bg-white dark:hover:bg-white/5")}><ShieldAlert size={15} /></button>
 <button onClick={fetchPosts} className="p-2 text-slate-400 hover:text-pink-500"><RefreshCw size={15} className={loadingPosts ? '' : ''} /></button>
 </div>
 </div>
 ...
 <div key={post.id} onClick={() => { setSelectedPost(post); setView('chat'); }} className={cn("p-4 cursor-pointer border-b border-slate-100 dark:border-white/5 flex items-start gap-3 group relative", selectedPost?.id === post.id ? "bg-white dark:bg-[#16161d] " : "hover:bg-white/50 dark:hover:bg-white/5")}>
 <div className="w-12 h-12 rounded-none overflow-hidden bg-slate-100 dark:bg-white/10 shrink-0 border border-slate-200 dark:border-white/10 ">
 ...
 {(post.auto_reply_enabled || post.auto_dm_enabled) && <Zap size={10} className="text-emerald-500 " />}
 </div>
 </div>
 {selectedPost?.id === post.id && <div className="absolute left-0 top-4 bottom-4 w-1 bg-pink-500 rounded-none -[0_0_8px_rgba(236,72,153,0.5)]" />}
 </div>
 ))}
 </div>
 </div>
 ...
 <div className={cn(
 "flex-1 flex flex-col bg-white dark:bg-[#050505] relative overflow-hidden ",
 ...
 <button onClick={() => setShowGuard(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-none text-slate-400"><X size={20} /></button>
 </div>
 <div className="p-6 flex-1 overflow-y-auto no-scrollbar space-y-8">
 <div className="flex items-center justify-between p-5 bg-pink-500/5 border border-pink-500/10 rounded-none">
 <div className="flex items-center gap-4">
 <div className={`w-10 h-10 rounded-none flex items-center justify-center ${autoDelete ? 'bg-pink-500 text-white -pink-500/20' : 'bg-slate-200 dark:bg-white/10 text-slate-400'}`}>
 <ShieldAlert size={20} />
 </div>
 <div>
 ...
 <button onClick={toggleAutoDelete} className={`w-12 h-6 rounded-none p-1 ${autoDelete ? 'bg-pink-500' : 'bg-slate-300 dark:bg-white/10'}`}>
 ...
 <input value={newKeyword} onChange={(e) => setNewKeyword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddKeyword()} placeholder="Add word..." className="flex-1 h-12 bg-white dark:bg-[#1a1a25] border border-slate-100 dark:border-white/5 rounded-none px-4 text-sm outline-none text-slate-900 dark:text-white focus:ring-1 focus:ring-pink-500" />
 <button onClick={handleAddKeyword} className="w-12 h-12 flex items-center justify-center bg-black dark:bg-white text-white dark:text-black rounded-none hover:scale-105 "><Plus size={20} /></button>
 </div>
 <div className="flex flex-wrap gap-2 pt-2">
 {keywords.map((word) => (
 <div key={word} className="flex items-center gap-2 bg-white dark:bg-white/5 px-4 py-2 rounded-none border border-slate-200 dark:border-white/5 group">
 ...
 <button onClick={() => setShowAutoManager(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-none text-slate-400"><X size={20} /></button>
 </div>
 <div className="p-6 flex-1 overflow-y-auto no-scrollbar space-y-6 pb-0">
 <div className="p-5 bg-white dark:bg-[#16161d] border border-slate-100 dark:border-white/5 rounded-none space-y-4">
 <span className="text-[10px] font-black text-pink-500 uppercase tracking-widest">Trigger Keywords</span>
 <input value={triggerKeyword} onChange={(e) => setTriggerKeyword(e.target.value)} placeholder="Price, Link (Empty for all)..." className="w-full h-12 bg-slate-50 dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 rounded-none px-4 text-xs outline-none text-slate-900 dark:text-white focus:ring-1 focus:ring-pink-500" />
 </div>
 
 <div className="space-y-4">
 <div className="flex items-center justify-between"><span className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Public Reply</span><button onClick={() => setIsAutoReplyEnabled(!isAutoReplyEnabled)} className={cn("w-12 h-6 rounded-none relative ", isAutoReplyEnabled ? "bg-pink-500" : "bg-slate-200 dark:bg-white/10")}><div className={cn("absolute top-1 w-4 h-4 bg-white rounded-none ", isAutoReplyEnabled ? "left-7" : "left-1")} /></button></div>
 <textarea value={autoReplyText} onChange={(e) => setAutoReplyText(e.target.value)} placeholder="Write public reply..." className="w-full h-28 bg-white dark:bg-[#16161d] border border-slate-100 dark:border-white/5 rounded-none p-4 text-xs outline-none text-slate-900 dark:text-white resize-none focus:ring-1 focus:ring-pink-500" />
 </div>
 ...
 <textarea value={autoDmText} onChange={(e) => setAutoDmText(e.target.value)} placeholder="Write private DM content..." className="w-full h-28 bg-white dark:bg-[#16161d] border border-slate-100 dark:border-white/5 rounded-none p-4 text-xs outline-none text-slate-900 dark:text-white resize-none focus:ring-1 focus:ring-emerald-500" />
 </div>

 <div className="p-4 bg-pink-500/5 border border-pink-500/10 rounded-none space-y-3">
 ...
 className="w-full h-12 bg-white dark:bg-[#1a1a25] border border-pink-200 dark:border-white/10 rounded-none px-4 text-xs outline-none text-slate-900 dark:text-white appearance-none cursor-pointer"
 >
 ...
 <button onClick={handleSaveAutomation} className="w-full h-14 bg-black dark:bg-white text-white dark:text-black rounded-none font-black text-[10px] uppercase tracking-[0.2em] hover:scale-[1.02] ">Save IG automation</button>
 </div>
 </motion.div>
 )}
 </AnimatePresence>
 ...
 <div className="w-10 h-10 rounded-none overflow-hidden border border-slate-200 dark:border-white/5 shrink-0">
 ...
 <button onClick={() => setShowAutoManager(true)} className={cn("flex items-center gap-2 px-4 py-2 rounded-none text-[9px] font-black uppercase ", (selectedPost.auto_reply_enabled || selectedPost.auto_dm_enabled) ? "bg-pink-500 text-white" : "bg-white dark:bg-white/5 text-slate-400 border border-slate-200 dark:border-white/5")}><Bot size={14} /><span className="hidden xs:inline">Automate</span></button>
 <a href={selectedPost.permalink} target="_blank" className="p-2 text-slate-400 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/5 rounded-none hover:text-pink-500 "><ExternalLink size={16} /></a>
 </div>
 </div>
 ...
 {loadingComments ? (
 <div className="flex flex-col items-center justify-center py-20"><RefreshCw className=" text-pink-500" size={24} /></div>
 ) : comments.length === 0 ? (
 ...
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={comment.id} className="bg-white dark:bg-[#16161d] p-5 rounded-none border border-slate-100 dark:border-white/10 relative group">
 <div className="flex items-start gap-4">
 <div className="w-10 h-10 rounded-none bg-gradient-to-tr from-pink-500 to-yellow-500 flex items-center justify-center text-white text-xs font-black shrink-0 ">
