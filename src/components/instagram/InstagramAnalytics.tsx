...
export const InstagramAnalytics = ({ user, account }: { user: any; account: any }) => {
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 bg-pink-500 text-white rounded-none text-[10px] font-bold uppercase hover:opacity-90 disabled:opacity-50 ">
 <RefreshCw size={12} className={loading ? '' : ''} /> Refresh from Meta
 </button>
 </div>
 ...
 className="bg-white dark:bg-[#16161d] p-4 rounded-none border border-slate-200 dark:border-white/5 hover:border-pink-400 ">
 <div className="flex items-center justify-between mb-3">
 <div className="w-9 h-9 rounded-none flex items-center justify-center text-white" style={{ background: kpi.color }}>
 {React.cloneElement(kpi.icon as React.ReactElement, { size: 16 })}
 </div>
 {loading && <RefreshCw size={12} className="text-slate-300 " />}
 </div>
 ...
 <div className="h-1.5 bg-slate-100 dark:bg-white/5 rounded-none overflow-hidden">
 <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, ease: 'easeOut' }}
 className="h-full rounded-none" style={{ background: item.color }} />
 </div>
 </div>
 );
 })}
 ...
 {loading ? (
 <div className="flex justify-center py-10"><RefreshCw className="w-6 h-6 text-pink-500 " /></div>
 ) : media.length > 0 ? (
 ...
 {media.map(post => (
 <tr key={post.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 ">
 <td className="py-3">
 <div className="flex items-center gap-2">
 {(post.thumbnail_url || post.media_url) ? (
 <img src={post.thumbnail_url || post.media_url} className="w-9 h-9 rounded-none object-cover" alt="" />
 ) : (
 <div className="w-9 h-9 rounded-none bg-slate-100 dark:bg-white/10 flex items-center justify-center">
 ...
 <td className="py-3">
 <span className={`px-2 py-0.5 rounded-none text-[8px] font-bold uppercase ${ post.media_type === 'VIDEO' ? 'bg-pink-100 text-pink-600 dark:bg-pink-900/20 dark:text-pink-400' : post.media_type === 'CAROUSEL_ALBUM' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400' }`}>
