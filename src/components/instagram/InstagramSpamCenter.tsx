...
export const InstagramSpamCenter = ({ user, account }: { user: any; account: any }) => {
 ...
 <div className="w-9 h-9 bg-pink-500/10 text-pink-500 rounded-none flex items-center justify-center ">
 <ShieldAlert size={18} />
 </div>
 <div>
 ...
 className={`flex-none px-3 md:px-4 py-2 md:py-1.5 rounded-none text-[9px] font-black uppercase tracking-widest ${!selectedMedia && !searchMedia ? 'bg-pink-600 text-white -pink-500/10' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}
 >
 All
 </button>
 ...
 className="w-full h-9 md:h-8 pl-8 pr-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-none text-[10px] md:text-[9px] font-bold outline-none text-slate-900 dark:text-white focus:border-pink-500 "
 />
 
 {showDropdown && (
 <div className="absolute top-full left-[-45px] md:left-0 right-0 w-[calc(100vw-24px)] md:w-full mt-1 bg-white dark:bg-[#1c1c26] border border-slate-200 dark:border-white/10 rounded-none z-50 max-h-64 overflow-y-auto no-scrollbar py-2">
 ...
 >
 <p className="text-[11px] md:text-[10px] font-black text-slate-900 dark:text-white uppercase group-hover:text-pink-500 ">Media ID: {m.id}</p>
 ...
 className="px-3 md:px-4 py-2 bg-rose-500 text-white rounded-none text-[9px] font-black uppercase tracking-widest hover:bg-rose-600 flex items-center gap-2"
 >
 {isBulkDeleting ? <RefreshCw size={12} className="" /> : <Trash2 size={12} />}
 ...
 <div key={item.id} className={`bg-white dark:bg-[#16161d] p-5 rounded-none border group ${selectedComments.includes(item.id) ? 'border-rose-500 ring-1 ring-rose-500/20' : 'border-slate-200 dark:border-white/5 hover:border-pink-500/30'}`}>
 <div className="flex items-start justify-between gap-4 mb-4">
 <div className="flex items-center gap-4">
 <button 
 onClick={() => toggleSelectComment(item.id)}
 className={`w-5 h-5 rounded-none border-2 flex items-center justify-center ${selectedComments.includes(item.id) ? 'bg-rose-500 border-rose-500 text-white' : 'border-slate-200 dark:border-white/20'}`}
 >
 {selectedComments.includes(item.id) && <CheckCircle size={12} />}
 </button>
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 bg-slate-100 dark:bg-white/5 rounded-none flex items-center justify-center text-slate-500 text-xs font-black">
 ...
 <div className="px-2.5 py-1 bg-rose-500/10 text-rose-500 rounded-none text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5">
 <AlertTriangle size={10} />
 Flagged: {item.matched_keyword}
 </div>
 </div>
 </div>
 <div className="p-4 bg-slate-50 dark:bg-black/20 rounded-none border border-slate-100 dark:border-white/5 mb-4">
 ...
 <div className="flex items-center gap-2">
 <button onClick={() => handleIgnore(item.id)} className="px-4 py-2 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 rounded-none text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-white/10 ">Ignore</button>
 <button onClick={() => handleDelete(item.id, item.commentId)} disabled={isDeleting === item.id} className="px-4 py-2 bg-rose-500 text-white rounded-none text-[9px] font-black uppercase tracking-widest hover:bg-rose-600 flex items-center gap-2">
 {isDeleting === item.id ? <RefreshCw size={12} className="" /> : <Trash2 size={12} />}
 Delete Spam
 </button>
 </div>
 </div>
