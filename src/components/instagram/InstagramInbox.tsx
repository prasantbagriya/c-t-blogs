...
export const InstagramInbox = ({ user, account }: { user: any, account: any, messages?: any[] }) => {
 ...
 <div className="w-16 h-16 bg-pink-500/10 text-pink-500 rounded-none flex items-center justify-center mb-6">
 <AlertCircle size={32} />
 </div>
 ...
 className="mt-8 px-8 py-3 bg-pink-600 text-white rounded-none font-black text-[10px] uppercase tracking-widest hover:bg-pink-700 -pink-500/20"
 >
 Reconnect account
 </button>
 </div>
 ...
 <button onClick={fetchConversations} className="p-2 text-slate-400 hover:text-pink-500 ">
 ...
 <div className="p-8 text-center"><RefreshCw className=" mx-auto text-slate-300" size={24} /></div>
 ...
 className={cn(
 "p-4 cursor-pointer border-b border-slate-100 dark:border-white/5 flex items-center gap-3",
 selectedConversation?.id === conv.id ? 'bg-white dark:bg-[#16161d] ' : 'hover:bg-white/50 dark:hover:bg-white/5'
 )}
 >
 <div className="w-11 h-11 bg-black dark:bg-white rounded-none flex items-center justify-center text-white dark:text-black font-black text-sm">
 ...
 {conv.unread_count > 0 && (
 <div className="w-2 h-2 bg-pink-500 rounded-none" />
 )}
 </div>
 ))
 )}
 ...
 <div className="w-8 h-8 bg-gradient-to-tr from-pink-500 to-yellow-500 rounded-none flex items-center justify-center text-white text-[10px] font-black ">
 ...
 <div className="w-1.5 h-1.5 bg-emerald-500 rounded-none -[0_0_8px_rgba(16,185,129,0.5)]" />
 ...
 <div className="flex items-center justify-center h-full"><RefreshCw className=" text-blue-500" /></div>
 ) : (
 messages.map(msg => {
 ...
 <div className={cn(
 "max-w-[85%] lg:max-w-[70%] px-3 py-1.5 rounded-none text-[13px] font-medium leading-snug",
 ...
 className="flex-1 bg-slate-100 dark:bg-[#16161d] border-none rounded-none px-4 py-3 text-[13px] font-medium text-slate-900 dark:text-white placeholder:text-slate-500 outline-none"
 />
 <button 
 onClick={handleSend}
 disabled={!replyText.trim()}
 className="p-3.5 bg-black dark:bg-white text-white dark:text-black rounded-none hover:scale-105 disabled:opacity-50 disabled:scale-100 dark:"
 >
