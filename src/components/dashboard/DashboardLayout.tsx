...
export default function DashboardLayout({
 ...
 <div className="w-9 h-9 bg-black dark:bg-white rounded-none flex items-center justify-center "><Zap className="text-white dark:text-black w-5 h-5 fill-current" /></div>
 ...
 <button key={item.id} onClick={() => setActiveTab(item.id)} className={cn("w-full flex items-center gap-3 px-4 py-2.5 rounded-none mb-1", activeTab === item.id ? "bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-white" : "text-slate-500 hover:bg-slate-50 dark:hover:bg-white/2")}>
 ...
 <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 bg-rose-500/10 text-rose-600 rounded-none font-black text-[10px] uppercase tracking-widest hover:bg-rose-500 hover:text-white "><LogOut size={16} />{isSidebarOpen && "Sign Out"}</button>
 </div>
 </aside>
 ...
 <button onClick={() => isMobile ? setIsMobileDrawerOpen(true) : setIsSidebarOpen(!isSidebarOpen)} className="p-2 border border-slate-100 dark:border-white/10 rounded-none text-slate-500 hover:text-black dark:hover:text-white "><Menu size={14} /></button>
 <div className="flex items-center gap-3">
 <div className="flex items-center">
 {selectedAccount ? (
 <div onClick={() => setActiveSubTab?.('overview')} className="flex items-center gap-2 px-2.5 py-1.5 border border-blue-500/20 dark:border-blue-500/10 rounded-none bg-blue-50/50 dark:bg-blue-500/5 backdrop-blur-sm cursor-pointer hover:border-blue-500 z-10 relative">
 <div className="w-6 h-6 rounded-none bg-black dark:bg-white flex items-center justify-center overflow-hidden border border-slate-200 dark:border-white/10 shrink-0">
 ...
 <span onClick={() => setActiveSubTab?.('overview')} className="text-sm font-black lowercase text-slate-900 dark:text-white cursor-pointer hover:text-blue-500 px-2 tracking-tight">{activeTab.replace('-', ' ')}</span>
 )}

 {/* Other accounts for same platform */}
 {currentPlatformAccounts.length > 1 && (
 <div className="flex items-center -ml-2 pl-4 pr-2 py-1 bg-slate-100/50 dark:bg-white/5 rounded-none border border-l-0 border-slate-100 dark:border-white/5">
 {currentPlatformAccounts.filter(acc => acc.id !== selectedAccount?.id).slice(0, 3).map(acc => (
 <button 
 key={acc.id} 
 onClick={() => setSelectedAccount?.(acc)}
 className="w-6 h-6 rounded-none border-2 border-white dark:border-[#0a0a0f] -ml-2 first:ml-0 hover:scale-110 hover:z-20 overflow-hidden bg-slate-200 dark:bg-white/10"
 ...
 <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 border border-slate-100 dark:border-white/10 rounded-none text-slate-500">{isDarkMode ? <Sun size={14} /> : <Moon size={14} />}</button>
 <button onClick={() => setIsNotifOpen(!isNotifOpen)} className="p-2 border border-slate-100 dark:border-white/10 rounded-none text-slate-500"><Bell size={14} /></button>
 <button onClick={() => setActiveTab('profile' as any)} className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-white/5 rounded-none pr-3">
 <div className="w-7 h-7 rounded-none bg-blue-500 flex items-center justify-center text-white font-black text-[10px] overflow-hidden">
 ...
 <nav className="h-16 bg-white dark:bg-[#0a0a0f] border-t border-slate-100 dark:border-white/5 flex items-center justify-around px-2 fixed bottom-0 left-0 right-0 z-100 -[0_-4px_20px_rgba(0,0,0,0.05)]">
 {(activeTab === 'threads' ? [
 ...
 { id: 'whatsapp', icon: <LayoutDashboard size={18} />, label: 'Hub', subTab: 'overview' },
 { id: 'whatsapp', icon: <MessageSquare size={18} />, label: 'Inbox', subTab: 'inbox' },
 { id: 'whatsapp', icon: <Send size={18} />, label: 'Broadcast', subTab: 'bulksend' },
 { id: 'whatsapp', icon: <CheckSquare size={18} />, label: 'Templates', subTab: 'templates' },
 { id: 'whatsapp', icon: <Settings size={18} />, label: 'Accounts', subTab: 'accounts' }
 ] : activeTab === 'widget' ? [
 ...
 return (
 <button key={item.label} onClick={() => { setActiveTab(item.id as DashboardTab); if ('subTab' in item && setActiveSubTab) setActiveSubTab(item.subTab as string); }} className={cn("flex flex-col items-center gap-0.5 p-2 ", isActive ? "text-blue-500 scale-110" : "text-slate-400")}>
 {item.icon}
 ...
 <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} className="fixed inset-y-0 left-0 w-72 z-110 bg-white dark:bg-[#16161d] flex flex-col ">
 ...
 {group.items.map(item => (
 <button key={item.id} onClick={() => { setActiveTab(item.id); setIsMobileDrawerOpen(false); }} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-none mb-1", activeTab === item.id ? "bg-slate-100 dark:bg-white/5 text-blue-600" : "text-slate-500 hover:bg-slate-50")}>
 <div style={{ color: activeTab === item.id ? item.color : 'inherit' }}>{item.icon}</div>
