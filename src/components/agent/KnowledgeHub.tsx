...
export const KnowledgeHub = ({ user }: { user: any }) => {
  ...
  if (!agent) {
    return (
      <div className="p-12 text-center bg-white dark:bg-[#16161d] rounded-lg border border-slate-200 dark:border-white/5">
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-blue-600 p-8 rounded-lg text-white relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <button 
            onClick={handleSync}
            disabled={isSyncing}
            className={`px-8 py-3 bg-white text-blue-600 rounded font-bold text-xs uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-white/10 dark:hover:text-white transition-colors flex items-center gap-2 disabled:opacity-50`}
          >
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white dark:bg-[#16161d] p-6 rounded-lg border border-slate-200 dark:border-white/5">
            ...
            <div className="space-y-3">
              <div className="p-4 bg-slate-50 dark:bg-[#1a1a24] rounded border border-slate-100 dark:border-white/5 flex items-center gap-3">
              ...
              <button className="w-full p-4 bg-white dark:bg-[#16161d] border border-dashed border-slate-200 dark:border-white/10 rounded text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-blue-600 transition-colors flex items-center justify-center gap-2">
                <Plus size={14} /> Add Document
              </button>
            </div>
          </div>
          
          <div className="bg-[#0f0f13] p-6 rounded-lg text-white border border-white/5">
        ...
        <div className="lg:col-span-3 space-y-4">
           <div className="flex gap-2">
              <button 
                ...
                className={`px-6 py-2 rounded text-[10px] font-bold uppercase tracking-widest transition-colors ${activeTab === 'facts' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 text-slate-400 transition-all'}`}
              >
                Facts
              </button>
              <button 
                onClick={() => setActiveTab('sources')}
                className={`px-6 py-2 rounded text-[10px] font-bold uppercase tracking-widest transition-colors ${activeTab === 'sources' ? 'bg-blue-600 text-white' : 'bg-white dark:bg-[#16161d] border border-slate-200 dark:border-white/5 text-slate-400 transition-all'}`}
              >
                Assets
              </button>
           </div>

           <div className="bg-white dark:bg-[#16161d] rounded-lg border border-slate-200 dark:border-white/5 overflow-hidden min-h-[500px]">
              <div className="p-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                 <div className="relative w-64">
                    ...
                    <input 
                      ...
                      className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-[#1a1a24] border border-slate-100 dark:border-white/5 rounded text-xs outline-none focus:border-blue-500 dark:text-white"
                    />
                 </div>
              ...
              <div className="p-4 space-y-2">
                 {Array.isArray(agent.knowledgeBase) && agent.knowledgeBase.map((fact: any, i: number) => (
                    <div 
                      key={i}
                      className="p-4 bg-white dark:bg-[#13131a] rounded border border-slate-100 dark:border-white/5 flex items-start gap-4 transition-colors hover:bg-slate-50 dark:hover:bg-[#1a1a24]"
                    >
                      <div className="p-2 border border-slate-50 rounded bg-slate-50 dark:bg-slate-800">
