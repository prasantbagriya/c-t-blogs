    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-700">
       ...
       <div className="flex items-center justify-between">
          <button 
            ...
            className="flex items-center gap-2 text-slate-500 hover:text-slate-900 dark:hover:text-white font-black text-[10px] uppercase tracking-widest transition-all group"
          >
            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to All Integrations
          </button>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* STATS CARDS */}
          <div className="bg-white dark:bg-[#16161d] p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-2xl shadow-black/5 relative overflow-hidden group">
             ...
             <div className="mt-4 flex items-center gap-2">
                <span className="px-2 py-1 bg-emerald-100 text-emerald-600 text-[9px] font-black rounded-lg">+12.4%</span>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Growth vs Last Month</span>
             </div>
          </div>
          <div className="bg-white dark:bg-[#16161d] p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-2xl shadow-black/5 relative overflow-hidden group">
             <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:scale-110 transition-transform"><CreditCard size={64} /></div>
             ...
             <div className="mt-4 flex items-center gap-2">
                <span className="px-2 py-1 bg-blue-100 text-blue-600 text-[9px] font-black rounded-lg">Healthy</span>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Recurring Revenue</span>
             </div>
          </div>
          <div className="bg-white dark:bg-[#16161d] p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-2xl shadow-black/5 flex flex-col justify-center gap-4 border-dashed border-blue-500/20">
             <button 
               onClick={() => setShowLinkModal(true)}
               className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all hover:scale-[1.02] active:scale-95 shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2"
             >
       ...
       {/* WEBHOOK SETUP CARD */}
       <div className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-blue-600/20 dark:to-blue-900/20 p-8 rounded-[2rem] border border-white/10 shadow-2xl relative overflow-hidden">
             ...
             <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                   <input 
                     ...
                     className="flex-grow px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-xs font-mono text-white outline-none"
                   />
                   <button 
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* REVENUE CHART */}
          <div className="bg-white dark:bg-[#16161d] rounded-[2rem] border border-slate-200 dark:border-white/5 overflow-hidden shadow-2xl shadow-black/5">
          ...
          {/* RECENT TRANSACTIONS */}
          <div className="bg-white dark:bg-[#16161d] rounded-[2rem] border border-slate-200 dark:border-white/5 overflow-hidden shadow-2xl shadow-black/5 flex flex-col">
            <div className="p-8 border-b border-slate-50 dark:border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-64">
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-white/5 rounded-xl outline-none text-[10px] font-bold dark:text-white border border-transparent focus:border-blue-500/50 transition-all"
                />
               ...
               <table className="w-full text-left">
                 ...
                 <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                    {payments.filter(p => 
                      ...
                      <tr key={pay.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-all group">
                        ...
                        <td className="px-8 py-5">
                           <span className={`px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-widest ${pay.status === 'paid' ? 'bg-emerald-100 text-emerald-600' : pay.status === 'refunded' ? 'bg-rose-100 text-rose-600' : 'bg-amber-100 text-amber-600'}`}>
                              {pay.status}
                           </span>
                        </td>
                           ...
                           <div className="flex justify-end gap-2">
                              {pay.status === 'paid' && (
                                <button 
                                  ...
                                  className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-500/5 rounded-xl transition-all opacity-0 group-hover:opacity-100"
                                  title="Refund"
                                >
                                  <RefreshCw size={14} />
                                </button>
                              )}
                              <button className="p-2 text-slate-400 hover:text-blue-500 rounded-lg transition-all opacity-0 group-hover:opacity-100">
