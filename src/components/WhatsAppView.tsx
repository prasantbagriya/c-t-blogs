...
export const WhatsAppView = ({
  ...
  if (!selectedAccount && (!allAccounts || allAccounts.length === 0) && activeSubTab !== 'widget') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px]">
        <div className="w-full max-w-2xl bg-white dark:bg-[#16161d]/80 backdrop-blur-xl py-12 px-10 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl shadow-emerald-500/5 text-center">
            <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center mx-auto mb-6 text-white shadow-lg">
                <Smartphone size={32} />
            </div>
            ...
            <button onClick={onConnectAccount} className="px-8 py-4 bg-emerald-500 text-white rounded-xl flex items-center justify-center gap-3 hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20 mx-auto">
              <Plus size={20} />
    ...
    <div className="flex flex-col w-full h-full">
      {isHub ? (
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          ...
          {activeSubTab === 'bulksend' && <WhatsAppBulkSendView user={user} onSuccess={() => {}} selectedAccount={selectedAccount} />}
          ...
          {activeSubTab === 'leads' && <SalesDashboard user={user} messages={messages} customerProfiles={customerProfiles} departments={departments} onChatSelect={(phone) => { setSelectedChatIdFromLeads(phone); setActiveSubTab('inbox'); }} />}
          ...
          {activeSubTab === 'campaigns' && (
            <div className="p-6 space-y-6">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button className="p-3 bg-white dark:bg-[#1a1a24] border border-slate-200 dark:border-white/5 text-slate-600 dark:text-white rounded-xl hover:text-blue-600 transition-all shadow-sm"><RefreshCw size={20} /></button>
                </div>
          ...
          {activeSubTab === 'settings' && selectedAccount && (
            <div className="p-8 space-y-8 max-w-4xl">
              <div className="flex items-center justify-between">
                ...
                <div className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2"><div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" /><span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Connected</span></div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="bg-white dark:bg-[#16161d] p-6 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm space-y-4">
                    ...
                    <p className="text-sm font-mono font-bold text-slate-700 dark:text-slate-200 break-all bg-slate-50 dark:bg-white/5 p-3 rounded-lg">{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
