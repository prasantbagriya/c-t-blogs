...
const FAQItem: React.FC<FAQItemProps> = ({ q, a, index }) => {
  ...
  return (
    <motion.div
      className={`relative rounded-2xl border-[1.5px] transition-all duration-300 overflow-hidden group ${
        isOpen 
          ? 'bg-blue-900/10 shadow-[0_0_20px_rgba(79,70,229,0.1)]' 
          : 'bg-white/5'
      }`}
      animate={{
        borderColor: [
    ...
    >
      ...
      >
        ...
        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
          isOpen ? 'bg-blue-600/20 text-blue-400 rotate-180' : 'bg-white/5 text-gray-500'
        }`}>
              ...
              <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all font-medium"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full bg-black/40 border border-white/10 rounded-2xl px-5 py-4 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all font-medium resize-none"
                  />
                </div>
                <button className="w-full bg-white text-black py-5 rounded-2xl font-bold text-lg hover:shadow-[0_0_30px_rgba(129,140,248,0.6)] transition-all flex items-center justify-center gap-2 border-none cursor-pointer">
                  Send Message
                </button>
              </form>
            </div>
