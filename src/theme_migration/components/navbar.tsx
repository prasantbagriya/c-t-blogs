    ...
    <header className="fixed top-6 left-0 right-0 z-50 w-full px-4 pointer-events-none">
      <div className="w-full max-w-7xl mx-auto pointer-events-auto">
        <motion.nav
          ...
          className="relative bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl"
        >
          ...
          <div className="px-4">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="hidden md:flex flex-1 justify-center items-center space-x-8">
                ...
                >
                  <button className="flex items-center gap-1.5 text-sm font-medium text-gray-400 hover:text-white py-4">
                    Company <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isCompanyOpen ? 'rotate-180' : ''}`} />
                  </button>
                  <AnimatePresence>
                    {isCompanyOpen && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute top-[80%] left-1/2 -translate-x-1/2 w-48 p-2 bg-slate-950 border border-white/10 rounded-2xl backdrop-blur-xl"
                      >
                        {companyList.map(comp => (
                          <button
                            key={comp.page}
                            onClick={() => handleNavClick(comp.page)}
                            className="flex items-center w-full p-3 rounded-xl hover:bg-white/5 text-left"
                          >
                ...
                >
                  <button className="flex items-center gap-1.5 text-sm font-medium text-gray-400 hover:text-white py-4">
                    Free Tools <ChevronDown className={`w-3.5 h-3.5 transition-transform ${isToolsOpen ? 'rotate-180' : ''}`} />
                  </button>
                  ...
                  <AnimatePresence>
                    {isToolsOpen && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute top-[80%] left-1/2 -translate-x-1/2 w-64 p-2 bg-slate-950 border border-white/10 rounded-2xl backdrop-blur-xl"
                      >
                        {toolsList.map(tool => (
                          <button
                            key={tool.page}
                            onClick={() => handleNavClick(tool.page)}
                            className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-white/5 group text-left"
                          >
              ...
              <div className="hidden md:flex items-center space-x-4">
                ...
                <button
                  ...
                  className="bg-white text-black px-5 py-2 rounded-xl font-bold hover:bg-gray-100 text-sm active:scale-95"
                >
                  Get Started
                </button>
              </div>
