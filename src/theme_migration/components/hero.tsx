      ...
      <AnimatePresence>
        {showVideo && (
          ...
          >
            ...
            >
              <button 
                className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white backdrop-blur-md "
                onClick={() => setShowVideo(false)}
              >
                <X className="w-6 h-6" />
              </button>
          ...
          <div className="flex flex-col gap-12 lg:items-start items-center">
            ...
            <div className="w-full flex flex-row gap-3 sm:gap-4 items-center justify-center lg:justify-start">
              <button onClick={() => onNavigate?.('auth')} className="group relative">
                <div className="absolute -inset-0.5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity bg-[linear-gradient(45deg,#4f46e5,#9333ea,#ec4899,#4f46e5)] bg-[length:200%_200%] animate-rainbow blur-[2px]" />
                <motion.div 
                  className="relative z-10 rounded-xl bg-white text-black px-4 sm:px-8 py-3.5 font-bold flex items-center justify-center gap-2 group-hover:bg-gray-50 text-xs sm:text-sm"
                  whileHover={{ rotateX: 10, y: -2 }}
                  style={{ transformStyle: 'preserve-3d' }}
                >
                  Get Started
                  <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group- transition-transform" />
                </motion.div>
              </button>

              <button onClick={() => setShowVideo(true)} className="group">
                <motion.div 
                  className="rounded-xl bg-white/5 backdrop-blur-md border border-white/10 text-white px-4 sm:px-8 py-3.5 font-bold flex items-center justify-center gap-2 hover:bg-white/10 text-xs sm:text-sm"
                  whileHover={{ rotateX: 10, y: -2 }}
                  style={{ transformStyle: 'preserve-3d' }}
                >
                  Watch Demo
              ...
              <div className="hidden lg:flex flex-wrap items-center gap-10">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <div className={`w-9 h-9 ${badge.color} rounded-lg flex items-center justify-center `}>
                      {badge.icon}
                    </div>
                    <div>
