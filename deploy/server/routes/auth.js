...
router.post('/x-w', decodePayload, cookieFallback, async (req, res) => {
  try {
    ...
    if (code && !accessToken) {
      ...
      let exchangeUrl = `https://graph.facebook.com/${META_API_VERSION}/oauth/access_token?client_id=${META_APP_ID}&redirect_uri=${encodeURIComponent(finalRedirectUri)}&client_secret=${META_APP_SECRET}&code=${code}`;
      let exchangeResponse = await fetch(exchangeUrl);
      
      if (!exchangeResponse.ok) {
        const errorData = await exchangeResponse.json();
        console.error(`[${timestamp}] Code Exchange Failed: ${JSON.stringify(errorData)}`);
        
        // Try fallback without redirect_uri if needed
        console.log(`[${timestamp}] Retrying without redirect_uri...`);
        exchangeResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/oauth/access_token?client_id=${META_APP_ID}&client_secret=${META_APP_SECRET}&code=${code}`);
      }
    ...
    const wabaResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/me/whatsapp_business_accounts?access_token=${accessToken}`);
    ...
    if (wabaData.data && wabaData.data.length > 0) {
      ...
      for (const waba of wabaData.data) {
        const pnResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${waba.id}/phone_numbers?access_token=${accessToken}`);
        if (pnResponse.ok) {
    ...
    if (!wabaId && META_APP_ID && META_APP_SECRET) {
      try {
        ...
        const debugRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/debug_token?input_token=${accessToken}&access_token=${META_APP_ID}|${META_APP_SECRET}`);
        ...
        if (wabaScope && wabaScope.target_ids && wabaScope.target_ids.length > 0) {
          ...
          for (const tid of wabaScope.target_ids) {
            const pnResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${tid}/phone_numbers?access_token=${accessToken}`);
            if (pnResponse.ok) {
              const pnData = await pnResponse.json();
              if (pnData.data && pnData.data.length > 0) {
                wabaId = tid;
    ...
    const isComplete = Boolean(wabaId && phoneNumberId);
    
    const newAccount = {
      id: accountId,
      uid: uid,
      createdBy: uid, // ENSURE createdBy is set for API filtering
      phoneNumber,
      phoneNumberId,
      wabaId,
      accessToken,
...
router.post('/x-i', decodePayload, cookieFallback, async (req, res) => {
  try {
    ...
    const pagesResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/me/accounts?fields=name,access_token,instagram_business_account&access_token=${accessToken}`);
    ...
    for (const page of pagesData.data || []) {
      if (page.instagram_business_account) {
        ...
        const igInfoResponse = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${igId}?fields=username,name,profile_picture_url&access_token=${accessToken}`);
