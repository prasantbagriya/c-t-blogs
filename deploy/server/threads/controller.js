...
export const exchangeCode = async (req, res) => {
  try {
    ...
    const profileRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/me?fields=id,username,threads_profile_picture_url,threads_biography,follower_count&access_token=${accessToken}`
    );
  ...
  try {
    ...
    try {
      ...
      const insightsRes = await fetch(
        `https://graph.threads.net/${THREADS_API_VERSION}/me/threads_insights?metric=${metrics}&access_token=${accessToken}`
      );
    ...
    try {
      let currentUrl = `https://graph.threads.net/${THREADS_API_VERSION}/me/threads?fields=like_count,reply_count,repost_count,quote_count&limit=100&access_token=${account.accessToken}`;
      let pagesFetched = 0;
...
export const getThreads = async (req, res) => {
  try {
    ...
    let url = `https://graph.threads.net/${THREADS_API_VERSION}/me/threads?fields=${fields}&limit=100&access_token=${account.accessToken}`;
...
export const publishPost = async (req, res) => {
  try {
    ...
    while (status !== 'FINISHED' && attempts < 20) {
       ...
       const statusRes = await fetch(`https://graph.threads.net/${apiV}/${creationId}?fields=status&access_token=${token}`);
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    const metrics = 'views,likes,replies,reposts,quotes';
    const insightsRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/me/threads_insights?metric=${metrics}&access_token=${account.accessToken}`
    );
    const insights = await insightsRes.json();

    // Fetch profile fields (follower_count, biography, etc)
    const profileRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/me?fields=id,username,threads_profile_picture_url,threads_biography,follower_count&access_token=${account.accessToken}`
    );
    ...
    try {
      let aggUrl = `https://graph.threads.net/${THREADS_API_VERSION}/me/threads?fields=like_count,reply_count,repost_count,quote_count&limit=100&access_token=${account.accessToken}`;
      let aggPages = 0;
...
export const getReplies = async (req, res) => {
  try {
    ...
    const repliesRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/${threadId}/replies?fields=${fields}&access_token=${account.accessToken}`
    );
...
export const getConversations = async (req, res) => {
  try {
    ...
    const convRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/me/conversations?fields=${fields}&access_token=${account.accessToken}`
    );
...
export const getMessages = async (req, res) => {
  try {
    ...
    const msgRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/${conversationId}/messages?fields=${fields}&access_token=${account.accessToken}`
    );
...
export const deleteThread = async (req, res) => {
  try {
    ...
    const deleteRes = await fetch(
      `https://graph.threads.net/${THREADS_API_VERSION}/${threadId}?access_token=${account.accessToken}`,
      { method: 'DELETE' }
    );
