...
/**
 * Robustly cleans and encodes a Facebook access token.
 */
const getCleanToken = (raw) => {
  if (!raw) return '';
  let token = '';
  if (typeof raw === 'string') {
    token = raw.trim();
  } else if (typeof raw === 'object') {
    token = (raw.accessToken || raw.access_token || raw.token || (raw.authResponse && raw.authResponse.accessToken) || '').toString().trim();
  }
  // Remove noise like quotes
