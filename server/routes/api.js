...
// DEBUG ROUTE: View all widget settings (Helpful for Hostinger sync issues)
router.get('/debug/db/widget_settings', async (req, res) => {
  try {
    const data = await getCollection('widget_settings');
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.post('/messages/send', async (req, res) => {
  try {
    ...
    if (source === 'instagram') {
      ...
      const url = `https://graph.facebook.com/v20.0/me/messages?access_token=${getCleanToken(account.pageAccessToken)}`;
      const response = await fetch(url, {
        method: 'POST',
    ...
    if (source === 'threads') {
      ...
      const apiV = 'v1.0';
      const url = isReply
        ? `https://graph.threads.net/${apiV}/${recipient}/replies?access_token=${getCleanToken(account.accessToken)}`
        : `https://graph.threads.net/${apiV}/me/threads?access_token=${getCleanToken(account.accessToken)}`;
...
router.post('/instagram/accounts/:id/sync', async (req, res) => {
  try {
    ...
    const igRes = await fetch(`https://graph.facebook.com/v20.0/${account.instagramId}?fields=${igFields}&access_token=${getCleanToken(token)}`);
    ...
    for (const account of userAccounts) {
      try {
        ...
        const fbRes = await fetch(
          `https://graph.facebook.com/v22.0/me/accounts?fields=name,access_token,category,fan_count,picture{url},instagram_business_account&access_token=${token}`
        );
...
router.post('/facebook/pages/connect', async (req, res) => {
  try {
    ...
    const accountData = {
      uid,
      pageId,
      pageAccessToken: decodeURIComponent(getCleanToken(pageAccessToken)), // Store cleaned but raw in DB
      pageName,
...
router.post('/widgets', async (req, res) => {
  try {
    ...
    console.log(`[WidgetAPI] Save request received for widget: ${settings.id || 'NEW'}, user: ${uid}`);
    
    if (settings.id) {
      // Update
      const existing = await getDoc('widget_settings', settings.id);
      if (!existing) {
        console.error(`[WidgetAPI] Widget not found: ${settings.id}`);
        return res.status(404).json({ error: 'Widget not found' });
      }
      if (existing.uid !== uid) {
        console.error(`[WidgetAPI] Access denied: existing.uid=${existing.uid}, req.uid=${uid}`);
        return res.status(403).json({ error: 'Access denied' });
      }
      await updateDoc('widget_settings', settings.id, settings);
      console.log(`[WidgetAPI] Successfully updated widget: ${settings.id}`);
