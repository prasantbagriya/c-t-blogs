...
router.post('/', async (req, res) => {
  try {
    ...
    // DEBUG: Always log ALL incoming webhook payloads to console and file
    console.log(`[Webhook] RECEIVED POST at ${new Date().toISOString()}:`, JSON.stringify(body).substring(0, 500));
    try {
      const fs = await import('fs');
      const logMsg = `[${new Date().toISOString()}] RAW_WEBHOOK_POST: ${JSON.stringify(body)}\n`;
    ...
    if (body.object === 'whatsapp_business_account' || body.object === 'page' || body.object === 'instagram') {
      console.log(`[Webhook] Detected ${body.object} event`);
        ...
        for (const change of entry.changes || []) {
          ...
          if (value.messages) {
            for (const message of value.messages) {
              ...
              console.log(`[Webhook] Message from ${from} (${message.type}) | WABA: ${wabaId} | PhoneID: ${phoneId}`);
              ...
              if (text || interactiveId) {
                ...
                let account = accounts.find(acc => acc.wabaId === String(wabaId));
                
                if (!account && phoneId) {
                  account = accounts.find(acc => acc.phoneNumberId === String(phoneId));
                }

                // Auto-heal wabaId if matched via phoneId
                if (account && account.wabaId !== String(wabaId)) {
                   console.log(`[Webhook] 🩹 Auto-healing wabaId for account ${account.id} -> ${wabaId}`);
                   await updateDoc('whatsapp_accounts', account.id, { wabaId: String(wabaId) }).catch(() => {});
                }
                
                if (account) {
                  console.log(`[Webhook] ✅ Matched account ${account.id} for wabaId ${wabaId}`);
                  ...
                  console.warn(`[Webhook] ❌ No WhatsApp account found for wabaId: ${wabaId}. Available wabaIds: ${accounts.map(a => a.wabaId).join(', ')}`);
                }
              }
            }
          }
