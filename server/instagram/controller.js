...
export const exchangeCode = async (req, res) => {
  try {
    ...
    let rawInput = decodeURIComponent(getCleanToken(directToken || code || ''));
    ...
    const meRes = await fetch(`https://graph.facebook.com/${META_API_VERSION || 'v22.0'}/me?fields=id,name&access_token=${getCleanToken(accessToken)}`);
    ...
    const pagesRes = await fetch(`https://graph.facebook.com/${META_API_VERSION || 'v22.0'}/me/accounts?fields=name,access_token,instagram_business_account&access_token=${getCleanToken(accessToken)}`);
    ...
    try {
       const igRes = await fetch(`https://graph.facebook.com/${META_API_VERSION || 'v22.0'}/me/instagram_business_accounts?fields=id,username,name&access_token=${getCleanToken(accessToken)}`);
    ...
    if (pagesRes.ok && pagesData.data) {
      ...
      for (const page of pagesData.data) {
        ...
        if (!igId) {
           try {
             const subRes = await fetch(`https://graph.facebook.com/${META_API_VERSION || 'v22.0'}/${page.id}/instagram_accounts?access_token=${getCleanToken(page.access_token)}`);
        ...
        if (igId) {
           ...
           const igRes = await fetch(`https://graph.facebook.com/${META_API_VERSION || 'v22.0'}/${igId}?fields=${igFields}&access_token=${getCleanToken(page.access_token)}`);
...
export const syncAccount = async (req, res) => {
  try {
    ...
    const igFields = 'username,name,biography,website,profile_picture_url,followers_count,follows_count,media_count,account_type,is_verified';
    const igRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${igId}?fields=${igFields}&access_token=${getCleanToken(token)}`);
    const igInfo = await igRes.json();
    ...
    if (isVideo || contentType === 'reel') {
      ...
      while (status !== 'FINISHED' && attempts < 20) {
        ...
        const pollRes = await fetch(
          `https://graph.facebook.com/${apiVersion}/${containerId}?fields=status_code&access_token=${getCleanToken(token)}`
        );
...
export const getPosts = async (req, res) => {
  try {
    ...
    const mediaRes = await fetch(
      `https://graph.facebook.com/${apiV}/${igId}/media?fields=${fields}&limit=30&access_token=${getCleanToken(token)}`
    );
...
export const getComments = async (req, res) => {
  try {
    ...
    const commentsRes = await fetch(
      `https://graph.facebook.com/${apiV}/${postId}/comments?fields=${fields}&limit=50&access_token=${getCleanToken(token)}`
    );
...
export const deleteComment = async (req, res) => {
  try {
    ...
    const delRes = await fetch(
      `https://graph.facebook.com/${apiV}/${id}?access_token=${getCleanToken(token)}`,
      { method: 'DELETE' }
    );
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    const insightsRes = await fetch(
      `https://graph.facebook.com/${apiV}/${igId}/insights?metric=${insightMetrics}&period=day&since=${Math.floor((Date.now() - 7 * 24 * 3600 * 1000) / 1000)}&until=${Math.floor(Date.now() / 1000)}&access_token=${getCleanToken(token)}`
    );
    ...
    const mediaRes = await fetch(
      `https://graph.facebook.com/${apiV}/${igId}/media?fields=${mediaFields}&limit=12&access_token=${getCleanToken(token)}`
    );
    const mediaData = await mediaRes.json();
...
export const optimizeProfile = async (req, res) => {
  try {
    ...
    const profileRes = await fetch(
      `https://graph.facebook.com/${apiV}/${igId}?fields=username,biography,followers_count,media_count&access_token=${getCleanToken(token)}`
    );
